...
class Sms extends Model
{
     ...
     * @inheritdoc
     */
    public function setSmsTemplate($to, $cardName,$code)
    {
        if($code == '6fKf04'){
            $var = ['counselor' => $cardName['counselor'] ,'mobile' => $cardName['mobile']];
        }else{
...
//        $path == "47.93.202.248"
        ...
        if($path == "wechat.xingfufit.com" || $path ==  "saas.xingfufit.com" || $path == "preview.xingfufit.com"){
       ...
       if(is_array($smsInfo['templates']) && !empty($smsInfo['templates'])){
           ...
           switch($type)
           {
               ...
               case '12'         :
                   ...
                   break;
               case '13'         :
                   $dataInfo     = str_replace(['@var(classTime)','@var(oldCoach)','@var(oldClass)','@var(newCoach)','@var(newClass)'],[$varData['classTime'],$varData['oldCoach'],$varData['oldClass'],$varData['newCoach'],$varData['newClass']],$hh);//NhLmQ1
                   break;
               case '14'         :
                   $dataInfo     = str_replace(['@var(counselor)','@var(mobile)'],[$varData['counselor'],$varData['mobile']],$hh);//YtV4L 柜子到期提醒
                   break;
               default;
           }
           return $dataInfo;
...
//            "project"=>'TlIkY',
    ...
    {
        $this->setSmsTemplate($to,$name,$code);
        $url = $this->url;
        $post_data = array (
            "appid" => $this->appid,
            "to" => $this->to,
        ...
        $num = $code == '6fKf04' ? '14' : '12' ;
        $this->commonData($url,$post_data,$num,'');   //发送
    }
    /**
     * @迈步-办卡-发送短信
     * @create lihuien
     ...
     * @inheritdoc
     */
    public function sendUpdateLeague($to,$classTime,$oldCoach,$newCoach,$oldClass,$newClass)
    {
        $this->setUpdateLeague($to,$classTime,$oldCoach,$newCoach,$oldClass,$newClass);
        $url = $this->url;
        $post_data = array(
            "appid" => $this->appid,
            "to" => $this->to,
        ...
        $this->commonData($url, $post_data, '13', '');   //发送
    }
    /**
     * @团课排课 - 已有人预约的团课修改课程或教练 - 给参数赋值
     * @author zhumengke <zhumengke@itsports.club>
     * @create 2018/04/10
     * @inheritdoc
     */
    public function setUpdateLeague($to,$classTime,$oldCoach,$newCoach,$oldClass,$newClass)
    {
        $var = ['classTime' => $classTime,'oldCoach'=>$oldCoach,'oldClass'=>$oldClass,'newCoach'=>$newCoach,'newClass'=>$newClass];
        $var = json_encode($var);
        $this->vars = $var;
        $this->to = $to;
        $this->timestamp = time();
