...
class WipeData extends Model
{
     ...
     * @return boolean
     */
    public function updateCardStatus($venueId)
    {
        $cardData = \backend\models\MemberCard::find()
            ->where(['and',['status'=>1],['<','invalid_time',time()]])
            ->andWhere(['venue_id'=>$venueId])
            ->asArray()
            ->all();
        if (!empty($cardData)) {
            foreach ($cardData as $key => $value) {
                $cardOne = MemberCard::findOne(['id' => $value['id']]);
                $cardOne->status = 6;
                if (!$cardOne->save()) {
...
//                        var_dump($venueLimit);
     ...
     * @return array|bool
     */
    public function sendMaiBuMemberCode($vId, $code,$type='send')
    {
        $number = 0;
        if ($code == 'uiqq22') {
            $str = [];
        ...
        $counselorArr = ['郭兆坤' => '小郭','范世纪' => '小范','赵银豪' => '小赵'];
        /*有效会员
        $data = \backend\models\Member::find()->alias('member')
            ->joinWith(['memberCard mc'])
            ->joinWith(['memberDetails md'])
            ->where(['member.venue_id' => $vId,'member.member_type'=>1])
            ->andWhere(['not in', 'mobile', [0, '0', '*']])
            ->andWhere(['<=', 'md.birth_date', '1988-12-30'])
            ->andWhere(['>', 'invalid_time', time()])
            ->groupBy(['mc.member_id'])
            ->asArray()->all();
        */
        $employeeOneObj = \common\models\Employee::findOne(['id'=>$vId]);
        if(empty($employeeOneObj)) return '员工不存在';
        $str['counselor'] = $counselorArr[$employeeOneObj->name];
        $str['mobile']    = $employeeOneObj->mobile;
        $data = \backend\models\Member::find()->where(['counselor_id'=>$vId])->andWhere(['not in', 'mobile', [0, '0', '*']])->all();
//        Func::comeMemberSendCode('15225287234','',$code);
//        $data = SmsRecord::find()->where(['send_code' => $code,'venue_id'=>12])->andWhere(['between','id',103020,103996])->asArray()->all();
//        var_dump($data);die();
//        echo '2222';
        if($type == 'total') {
            return count($data);
...
//            $memberCard->another_name        = $cardCategory->another_name;          //另一个卡名
     ...
     * @return boolean
     */
    public function insertIcBind()
    {
        $icBind = \backend\models\MemberDetails::find()->where(['IS NOT','ic_number',null])->asArray()->all();
        foreach ($icBind as $key => $value) {
            $result = IcBindingRecord::find()->where(['and',['ic_number' => $value['ic_number']],['status' => 1]])->asArray()->all();
            if (count($result) > 0) {
                continue;
            }

            $model = new IcBindingRecord;
            $model->member_id            = $value['member_id'];
            $model->ic_number            = $value['ic_number'];
            $model->custom_ic_number     = (string)time().(string)$value['member_id'];
