...
angular.module('App').controller('venueManagementCtrl',function($scope,$http,$timeout){
    ...
    $scope.getAllVenue = function(){
        $http.get('/site/get-auth-venue').then(function(response){
            ...
            $scope.selectVenueId = response.data[0].id;
    ...
    $scope.editEditSeatComplete = function(num,type){
        ...
        var SeatType1231 = $scope.editEditSeatType;
        //console.log();
    ...
    $scope.getHouse = function () {
        $http.get('/venue-management/room-info?venueId='+$scope.selectVenueId).then(function (result) {
    ...
    $scope.addHouse = function () {
        ...
        $http.get('/site/get-auth-venue').then(function (result) {
            $scope.allVenue = result.data;
        });
        $('#siteManagementAdd2').modal('show')
    };
    //删除房间
    $scope.delHouse = function () {
        $http.get('/venue-management/del-room?roomId='+ $scope.DCodeId).then(function (result) {
            $scope.getHouse();
            $scope.DCodeId = ''
        })
    };
    ...
    //修改教室模态框
    $scope.updateClassroom = function (id,name,room_id,classroom_area) {
        $scope.getHouse();
        $scope.classroomId = id;
        $scope.upName      = name;
        $scope.upSquare    = classroom_area;
        if(room_id == 0){
            $scope.upRoomId = '';
        }else{
            $scope.upRoomId = room_id;
        }
        $('#updateClassroomModel').modal('show');
    };
    //修改教室点击完成
    $scope.upClassroomComplete = function () {
        $scope.upClassroomData = function () {
            return {
                _csrf_backend : $('#_csrf').val(),
                classroomId   : $scope.classroomId,
                upRoomId      : $scope.upRoomId,
                upName        : $scope.upName,
                upSquare      : $scope.upSquare
            }
        };
        if($scope.upRoomId === '' || $scope.upRoomId === undefined){
            Message.warning('请选择房间');
            return;
        }
        if($scope.upName === '' || $scope.upName === undefined){
            Message.warning('请填写教室名称');
            return;
        }
        $http({
            url     : '/venue-management/update-class-room',
            method  : 'POST',
            data    : $.param($scope.upClassroomData()),
            headers : {'Content-Type': 'application/x-www-form-urlencoded'}
        }).then(function(response){
            if(response.data.status === 'success'){
                Message.success(response.data.data);
                $scope.getAllVenueSite();
                $('#updateClassroomModel').modal('hide');
            }else{
                Message.warning(response.data.data);
            }
        });
    ...
    $scope.addClassroomComplete = function(){
        $scope.addClassroomData = function(){
            return {
                _csrf_backend   : $('#_csrf').val(),
                roomId           : $scope.DCodeId ? $scope.DCodeId : null,
                venueId          : $scope.selectVenueId,
