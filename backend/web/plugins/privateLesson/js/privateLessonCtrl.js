...
app.controller('privateClassCtrl', function ($scope, $http, Upload, $rootScope, $interval) {
    ...
    $scope.getClassData = function () {
        $http.get("/private-teach/get-about-data?id=" + $scope.id + '&aboutId=' + $scope.courseId).then(function (result) {
            ...
            //获取今天的日期
            $scope.current_date = $scope.getMyDate(new Date().getTime());
    ...
    $scope.confirmFinishClass = function (mobile,id,end) {
        ...
        $scope.classStartTime  = $scope.memberDetail.start * 1000;
        $scope.classEndTime    = $scope.memberDetail.end * 1000;
        $scope.actualStartTime = $scope.memberDetail.actual_start * 1000;
    ...
    $scope.closeCompas2 = function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                //接口调用成功返回时接口调用成功返回时
                if (ret == 0 && data.score != undefined) {
                    if($scope.actualStartTime < $scope.classStartTime){    //提前登记上课
                        if ($scope.currentTime - $scope.actualStartTime > $scope.classEndTime - $scope.classStartTime) {
                            $scope.updateStatusF();
                        }else{
...
//私教课程页面
app.controller('privateLessonCtrl', function ($scope, $rootScope, $http, $timeout, Upload, $compile) {
    $(function(){
    ...
    $scope.getOldCourse = function(){
        ...
        oldLists.each(function(index){
            ...
            $courseLength.val(parseFloat(currentCourse.course_length));
            $courseNum.val(parseFloat(currentCourse.course_num));
            $originalPrice.val(parseFloat(currentCourse.original_price));
            $appOriginalPrice.val(parseFloat(currentCourse.app_original));
        });
    };
    //公共属性判断
    ...
    $scope.getOldCourseSection = function(){
        ...
        $courseSecBox.each(function (i) {
            ...
            var $appDiscount = $(this).find("input[name='appDiscount']");
            ...
            if($scope.privateClassDetails.price.length > 0){
                ...
                $appDiscount.val($priceData[i].app_discount);
            }
    ...
    $scope.getCourseSectionData = function(){
        ...
        $courseSecBox.each(function (i) {
            ...
            var $appDiscount = $(this).find("input[name='appDiscount']").val();
            if($startSec !=''){
            ...
            if($appDiscount !=''){
                $scope.appDiscountArr.push($appDiscount);
            }
        });
    };
    //动态填写区间开始
    ...
    $scope.courseSectionEdit = function(){
        ...
        var courseSection = {
            ...
            appDiscount    :$scope.appDiscountArr,//POS价
            _csrf_backend : $('#_csrf').val()
        };
    ...
    $scope.skipPage = function(value,id){
        if(value != undefined){
            $scope.pageInitUrl = '/private-teach/charge-class?page='+value;
            $scope.getData();
        }
    };
    ...
    //设置折扣按钮
    $scope.privateClassSetting = function () {
        $.loading.show();
        $('.setDiscountSmallBox').eq(0).siblings().remove();
        $scope.chooseRoleSelectArr = [];
        $http.get('/private-lesson/get-role').then(function (response) {
            $scope.roleListChoose = response.data;
        });
        $http.get('/private-lesson/get-set-discounts').then(function (result) {
            $scope.getDiscountList = result.data;
            if(result.data.length !== 0) {
                $timeout(function () {
                    var oldLists = $('.discountSettingBox').children('.setDiscountSmallBox');
                    for(var i = 0; i < oldLists.length; i++) {
                        $('.chooseDiscount').eq(i).val(angular.fromJson($scope.getDiscountList[i].category_id).join('/'));
                        $('.chooseRoleSelect').eq(i).val(parseInt($scope.getDiscountList[i].role_id));
                    }
                },1000);
                $timeout(function () {
                    $('.chooseRoleSelect').select2({
                        width : '100%'
                    });
                    $.loading.hide();
                    $('#setPrivateClassModal').modal('show');
                },1500);
            }else {
                $timeout(function () {
                    $('.chooseRoleSelect').select2({width : '100%'});
                    $.loading.hide();
                    $('#setPrivateClassModal').modal('show');
                },1000);
            }
    ...
    //新增折扣设置按钮
    $scope.chooseRoleSelectArr = [];
    $scope.setDiscountInputArr = [];
    $scope.num1 = 0;
    $scope.addNewDiscountHtml = function () {
        $.loading.show();
        $scope.htmlAttr1 = 'addNewDiscountSetting';
        $scope.num1  = $scope.num1 + 1;
        $http.get('/rechargeable-card-ctrl/add-venue?attr='+$scope.htmlAttr1+'&num='+$scope.num1).then(function (result) {
            $scope.setDiscountHtml = result.data.html;
            // $scope.privateClassSetting();
            $timeout(function(){
                $('.chooseRoleSelect').select2({
                    width: '100%'
                });
    ...
    //设置折扣确定按钮
    $scope.setPrivateClass = function () {
        $scope.chooseRoleSelectArr = [];
        $scope.setDiscountInputArr = [];
        var __pattern = new RegExp(/\//);
        for(var i = 0; i < $('.discountSettingBox').children().length; i++) {
            if(__pattern.test($('.discountSettingBox').children().eq(i).children().eq(0).children().eq(1).children().eq(0).val())) {
                $scope.chooseRoleSelectArr.push($('.discountSettingBox').children().eq(i).children().eq(0).children().eq(1).children().eq(0).val().split('/'));
            }else {
                $scope.chooseRoleSelectArr.push($('.discountSettingBox').children().eq(i).children().eq(0).children().eq(1).children().eq(0).val());
            }
            $scope.setDiscountInputArr.push($('.discountSettingBox').children().eq(i).children().eq(1).children().eq(1).children().val())
        }
        var request = {
            discounts       : $scope.chooseRoleSelectArr,
            roleId          : $scope.setDiscountInputArr,
            _csrf_backend   : $('#_csrf').val()
        };
        for(var j = 0; j < $scope.chooseRoleSelectArr.length; j++) {
            if(!$scope.chooseRoleSelectArr[j]) {
                Message.warning('您的折扣输入不完整！');
                $scope.chooseRoleSelectArr = [];
                $scope.setDiscountInputArr = [];
                return ;
            }
        }
        for(var k = 0; k < $scope.setDiscountInputArr.length; k++) {
            if(!$scope.setDiscountInputArr[k]) {
                Message.warning('您的角色选择不完整！');
                $scope.chooseRoleSelectArr = [];
                $scope.setDiscountInputArr = [];
                return ;
            }
        ...
        $scope.setDiscountInputArr = [];
        $http({
            url     : "/private-lesson/set-charge-discounts",
            method  : 'POST',
            data    : $.param(request),
            headers : {'Content-Type': 'application/x-www-form-urlencoded'}
        }).then(function (response) {
            if(response.data.status == 'success') {
                Message.success('设置折扣成功！');
                $scope.init();
                $('#setPrivateClassModal').modal('hide');
            }else {
                Message.warning('设置折扣失败，请刷新后重试！');
                $scope.init();
            }
        })
    };
    ...
    //点击删除按钮
    $scope.removeDiscountHtml = function (id) {
        if(id === undefined) {
            var __pattern = new RegExp(/\//);
            for(var i = 0; i < $('.discountSettingBox').children().length; i++) {
                if(__pattern.test($('.discountSettingBox').children().eq(i).children().eq(0).children().eq(1).children().eq(0).val())) {
            ...
            swal({
                ...
                if (isConfirm) {
                    $.loading.show();
                    for(var j = 0; j < $('.removeDiscountBtn').length; j++) {
                        if(parseInt(id) === parseInt($('.removeDiscountBtn').eq(j).data('remove-index'))) {
                            $scope.removeDiscountBtnId = $('.removeDiscountBtn').eq(j).data('zhangyaxin-id');
                            $scope.removeDiscountNum = j;
                        }
                    }
                    $timeout(function () {
                        $http.get('/member-card/del-gift-data?giftId=' + $scope.removeDiscountBtnId).then(function (response) {
                            if(!response) {
                                swal("删除失败！", "","error");
                                $.loading.hide();
                                return false;
                            }else {
                                $('.removeDiscountBtn').eq($scope.removeDiscountNum).parent().parent().parent().remove();
                                $.loading.hide();
                                swal("删除成功！", "","success");
                            }
                        });
...
app.controller('coachLessonCtrl', function ($scope, $rootScope, $http, $location, $timeout) {
    ...
    $scope.blueDate = function (time) {
        ...
        if($scope.selectPrivateClassId !== undefined){
    ...
    $scope.completeOrderCourse = function () {
        ...
        $scope.orderCourse = {
            _csrf_backend : $('#_csrf').val(),
            classId       : $scope.selectPrivateClassId,
            aboutType     : 'mobile',
        ...
        if ($scope.selectPrivateClassId === '' || $scope.selectPrivateClassId === undefined) {
            Message.warning("请选择预约课程");
            return;
        }
        if($scope.startClass === '' || $scope.startClass === undefined){
            Message.warning("上课时间不能为空");
            return;
        }
        if($scope.endClass === '' || $scope.endClass === undefined){
            Message.warning("下课时间不能为空");
            return;
        }
        //判断当前时间和预约时间
    ...
    $scope.chooseMemberCardIdOk = function(memberCardId) {
        ...
        $http.get('/private-teach/member?keyword=' + memberCardId).then(function (result) {
            if(result.data.status == 'success'){
                $scope.searchMemberDetail = result.data.data;
                var nowTime = new Date().getTime()/1000;
                if($scope.searchMemberDetail.status == 1 && nowTime > $scope.searchMemberDetail.invalid_time){
                    Message.warning('会员卡已到期！');
                    return;
                }
                if($scope.searchMemberDetail.status == 2){
                    Message.warning('会员卡状态异常！');
                    return;
                }
                if($scope.searchMemberDetail.status == 3){
                ...
                $scope.AutomaticThawingMember(result.data.data.memberDetails.member_id);
                $scope.privateCoursePic = null;
                $scope.selectPrivateClassId = '';
                $scope.startClass = '';
                $scope.endClass = '';
