...
angular.module('App').controller('leagueCtrl', function ($scope, $http, Upload,$timeout) {
    ...
    $scope.courseDetails = function (id,Coursestart) {
        ...
        $http({
            ...
            //console.log(success.data.data.aboutClass)
    ...
    $scope.revisedCurriculum = function () {
        if(!$scope.messageWarning($scope.courseDetailsID,"请选择课种")){ return}
        if(!$scope.messageWarning($scope.revisedCurriculumData.class_id,"请选择课程")){ return}
        if(!$scope.messageWarning($("#dayStartss").val(),"请选择开始时间")){ return}
        if(!$scope.messageWarning($("#dayEnd2").val(),"请选择结束时间")){ return}
        if ($("#dayStartss").val() != '' && $("#dayStartss").val() != null && $("#dayEnd2").val() != '' && $("#dayEnd2").val() != null) {
            var current = new Date().getTime();
            var currentTime = $scope.getMyDate(current);
            var start = currentTime + ' ' + $("#dayStartss").val() + ':00';
            var end = currentTime + ' ' + $("#dayEnd2").val() + ':00';
        ...
        if(!$scope.messageWarning($scope.revisedCurriculumData.coach_id,"请选择教练")){ return}
        if(!$scope.messageWarning($scope.revisedCurriculumData.classroom_id,"请选择教室")){ return}
        if(!$scope.messageWarning($scope.theSeatingOrderAllUpdateId,"请选择座次")){ return}
        var data = {
    ...
    $scope.changeCoachsClick = function (id, name, pic) {
        ...
        if(pic != ''){
            ...
            $scope.changeCoachsPic = "/plugins/user/images/pt.png";
        }
        $("#myModal4").modal("hide");
    };
    //鼠标进入
    ...
    $scope.classChange = function () {
        ...
        if(!$scope.messageWarning($scope.courseNameID,"请选择课程")){ return};
        if(!$scope.messageWarning($scope.dayStart,"请选择开始时间")){ return};
        if(!$scope.messageWarning($scope.dayEnd,"请选择结束时间")){ return};
    ...
    $scope.searchButton = function (data) {
        var keyword = (data == '' || data == null || data == undefined) ? '' : data;
        $http({method: 'get', url: '/new-league/group-class-data?course=' + keyword}).then(function (data) {
    ...
    $scope.appointmentSettingsNetClick = function () {
        ...
        console.log(data);
        if(data.before_class == undefined && data.cancel_time == undefined && data.personLowerLimit == undefined && data.printSettings == undefined){
            Message.warning('请将设置信息填写完整');
            return
        }
        if(data.before_class == '' && data.cancel_time == '' && data.personLowerLimit == '' && data.printSettings == ''){
            Message.warning('请将设置信息填写完整');
            return
        }
    ...
    $scope.clearSearchBtn = function(){
        ...
        $scope.missDetails($scope.missDetailsCardId);
    };
