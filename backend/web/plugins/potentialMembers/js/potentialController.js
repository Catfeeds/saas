...
/** 获取场馆**/
    ...
    $scope.addPaySelectElement = function () {
        ...
        $http({
            url: "/potential-members/add-venue",
            data: $.param({
                attr : $scope.attr,
                num  : $scope.num,
    ...
    //检测跨店购卡手机号是否重复的参数
    $scope.checkMemberInfo = function () {
        $scope.chooseVenueId = angular.fromJson($scope.venue).id;
        return{
            name     : $scope.name           != undefined && $scope.name            != "" ? $scope.name           : null, //姓名
            mobile   : $scope.mobile         != undefined && $scope.mobile          != "" ? $scope.mobile         : null, //手机
            venueId  : $scope.chooseVenueId  != undefined && $scope.chooseVenueId   != "" ? $scope.chooseVenueId  : null, //场馆
            _csrf_backend : $('#_csrf').val()
        }
    };
    ...
    $scope.addBuyCard = function () {
        ...
        $scope.getDepositAdd();
        //检测跨店购卡时手机号是否有重复
        //console.log($scope.checkMemberInfo());
        $http({
            url:'/sell-card/check-buy-card',
            method:'POST',
            data:$.param($scope.checkMemberInfo()),
            headers: {'Content-Type': 'application/x-www-form-urlencoded'}
        }).then(function(result){
            //console.log(result);
            console.log(result.data.status)
            // notBuyCard == 不是跨店签单 可以直接购卡
            if(result.data.status == 'notBuyCard'){
                Sweety.remove({
            ...
            }else if(result.data.status == 'noRepeat'){
                // 状态为noRepeat说明没有重复手机号 可以直接购卡
                Sweety.remove({
                    url : "/sell-card/confirm-sell-card",
                    http : $http,
                    title : '确定是否已收款?',
            ...
            }else if(result.data.status == 'allRepeat'){
                //状态为allRepeat说明有重复手机号 并且会员名相同
                swal({
                    title: "该场馆已存在相同手机号",
                    text: '该场馆已存在相同手机号,并且会员名相同,购卡成功后会员的基本信息会被覆盖，是否确定购卡？',
                    type: "warning",
            ...
            }else if(result.data.status == 'oneRepeat'){
                //状态为oneRepeat说明有重复手机号 但是会员姓名不同
                swal({
                    title: "该场馆已存在相同手机号",
                    text:'手机号相同,但会员姓名不同，购卡成功后会员的基本信息会被覆盖，是否确定购卡？',
                    type: "warning",
                    showCancelButton: true,
                    confirmButtonColor: "#DD6B55",
                    confirmButtonText: "确定",
...
/*** 潜在会员卡信息 ***/
    ...
    //最近入场记录 分页
    $scope.followRecordPages = function (urlPages) {
        // $scope.getFollowRecord(urlPages);
        $.loading.show();
        $http.get(urlPages).success(function (data){
            $scope.followRecord = data.data;
            $scope.followPages  = data.pages;
            if($scope.followRecord.length == 0){
                $scope.followRecordList = true;  //暂无数据图像显示
    ...
    $scope.getSendCardTimeRecord = function () {
        ...
        $http.get('/potential-members/information-records?memberId=' + $scope.potentialUserId).then(function (response) {
            ...
            if(response.data.data.length != 0){
                $scope.changeTimeLists = response.data.data;
                $scope.updateRecordList = false;
            }else{
                $scope.changeTimeLists = response.data.data;
                $scope.updateRecordList = true;
            }
            $.loading.hide();
        })
    };
    ...
    $scope.SelectMessage = function (value) {
        ...
        //最近入场记录
        if(value == '4'){
            $scope.getFollowRecord($scope.followUrl);
        }
        //赠卡记录
        if(value == '5') {
    ...
    $scope.depositInfoUpdate = function(object){
        ...
        console.log($scope.updateDepositType)
        if($scope.updateDepositType == 1){
    ...
    $scope.siteReservation = function (key,num) {
        ...
        //判断预约时间是否在7天内
        var choose = new Date($scope.startDate);
...
/*** 续费模块 ***/
    ...
    $scope.priceChange = function (){
        ...
        //console.log($scope.renCardRecondMoney)
    };
    ...
    $scope.renewData = function (){
        ...
        return{
            renewalWay    : $scope.renewTypeSelect != undefined && $scope.renewTypeSelect  != "" ? $scope.renewTypeSelect : null, //续费方式
            ...
            renewPrice    : $scope.renCardRecondMoney    != undefined  ? $scope.renCardRecondMoney    : null, //续费金额
            totalAmount   : $scope.renewPrice            != undefined  ? $scope.renewPrice    : null, //续费价格
    ...
    $scope.renewCardPost = function (){
        //保存数据之前数据验证
        if ($scope.renewTypeSelect == null || $scope.renewTypeSelect == "" || $scope.renewTypeSelect == undefined) {
            Message.warning("请选择续费方式");
            return false;
        }
        ...
        if ($scope.sellerSelect == null || $scope.sellerSelect == "") {
            Message.warning("请选择销售");
            return false;
        }
        if ($scope.chosePayType == null || $scope.chosePayType == "") {
            Message.warning("请选择付款方式");
            return false;
        }
        $scope.checkButton = true;
        //console.log( $scope.renewData())
        // 发送数据
        $http({
...
/***送人卡升级模块***/
// 升级卡的点击方法
    /*$scope.getMoreMoneyUpsateCard = function(){
    ...
    $scope.getUpsateCardType = function (data){
        ...
        if($scope.upsateType.sell_price == 'null' || $scope.upsateType.sell_price == null){
            ...
            $scope.upsateCardRecondMoney = $scope.newCardSellPrice - $scope.convertMoney; //升级价格 = 新卡价格 - 折算金额
    ...
    $scope.successUpdate = function () {
        ...
        if($scope.convertMoney == '' ||$scope.convertMoney == undefined || $scope.convertMoney == null ){
    ...
    $(".upsateCardMoneySelect").blur(
        function (){
            ...
            //$scope.upsateCardRecondMoney = parseInt($scope.upsateCardMoney) - $scope.convertMoney;
            $scope.upsateCardRecondMoney = parseInt($scope.upsateCardMoney);
            $('.newCardPricePay').text($scope.upsateCardRecondMoney.toFixed(2));
    ...
    //折算金额失去焦点
    $('.convertMoney').blur(function(){
        var reg = /^(([1-9]\d{0,3})|0)(\.\d{0,2})?$/ ;
        if(!reg.test($scope.convertMoney)){
            Message.warning('请输入有效的折算金额');
            $('.convertMoney').val('');
            return;
        }
        if(Number($scope.convertMoney) >= Number($scope.upsateCardMoney)){
            Message.warning('折算金额不能大于或者等于卡种价格');
            $('.convertMoney').val('');
            return
        }
        $scope.upsateCardRecondMoney = parseInt($scope.upsateCardMoney) - Number($scope.convertMoney);
        $('.newCardPricePay').text($scope.upsateCardRecondMoney.toFixed(2));
    });
    //折算金额change事件
    $scope.convertMoneyChange = function(){
        $scope.upsateMoneyVal = $(".upsateCardMoneySelect").val();
        $scope.upsateCardRecondMoney = parseInt($scope.upsateCardMoney);
        $('.newCardPricePay').text($scope.upsateCardRecondMoney.toFixed(2));
    };
    ...
    $scope.discountMath = function (){
        if($scope.getDiscountListValue != undefined && $scope.getDiscountListValue != ''){
            ...
            if($(".upsateCardMoneySelect").val() != ''&& $scope.discountCardList.discount != '' && $scope.discountCardList.discount != undefined  && $scope.discountCardList.discount != null){
                ...
                $scope.upsateCardRecondMoney = parseInt($(".upsateCardMoneySelect").val())*parseFloat($scope.discountCardList.discount)/10 - $scope.convertMoney;
                $('.newCardPricePay').text($scope.upsateCardRecondMoney.toFixed(2));
            }
        }
        else{
            $scope.discountId  = '';
            $scope.residueCard = '不限';
            $scope.upsateCardRecondMoney = parseInt($(".upsateCardMoneySelect").val()) - $scope.convertMoney;
