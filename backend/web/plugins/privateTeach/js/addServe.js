...
app.controller('addServeCtrl',function($scope,$http,$rootScope,Upload,$timeout){
    ...
    $scope.getPrice = function () {
        ...
        if($scope.commonProof($div)){
            $div.each(function (i) {
                ...
                if($appDiscount){
                    $scope.appDiscount.push($appDiscount);
                }
            });
        }else{
            $scope.price();
    ...
    $scope.getData = function () {
        ...
        return {
            ...
            appDiscount          : $scope.appDiscount,      //移动端折扣
    ...
    //计算app折扣之后的价格
    $scope.appDiscountPrice = 0;
    $scope.getAppDiscountPrice = function (appUnitPrice,appDiscounts) {
        if(appUnitPrice != undefined && appDiscounts != undefined){
            $scope.appDiscountPrice = appUnitPrice * appDiscounts / 10;
        }
    };
