...
app.controller('checkCardCtrl', function ($scope, $http, $rootScope, $filter, $timeout, Upload) {
    ...
    $scope.memberPath = function () {
        $scope.MAIN = {
            'MEMBER_PATH': {
                ...
                'memberYardRecord': "/member/get-member-about-yard-record?memberId=" + $scope.memberId,//场地记录
    ...
    $scope.getOrderSetting = function () {
        $http.get('/new-league/order-setting').then(function (result) {
                ...
                $scope.cancel_time  = result.data.cancel_time   != '' ? result.data.cancel_time   : 0;
        });
    }
        ...
        $timeout(function () {
            ...
            if($scope.allDayCourse == 'day'){
                ...
                if (open < 10) {
                    ...
                    //直接修改一天的课程打印小票
                    $scope.updateAboutStatus(id,'day');
                    window.print();
    ...
    //会员卡详情模态框关闭事件
    $('#myModals2').on('hidden.bs.modal', function () {
...
//点击修改页面的完成触发（修改手机号）
    ...
    //添加场地记录列表
    $scope.getYardRecord = function (val) {
        if (!val) {
            $scope.isLoading = true;
        }
        $http.get($scope.MAIN.MEMBER_PATH.memberYardRecord).then(function (response) {
...
// 到场离场记录日期插件调用
    ...
    $scope.memberChangeVenue = function (){
        $http.get("/member/member-change-venue?memberId=" + $scope.memberId).success(function (data){
            if (data.length > 0) {
    ...
    $scope.cancelReservation = function (id, num, key) {
        ...
        if (d < $scope.venueTime) {
            ...
            Sweety.remove({
                url: "/site-management/deal-yard-about-class?yardId="+$scope.initYardPageId+"&memberAboutDate="+$scope.venueTime+"&cardId="+$scope.siteReservationListDataCardId+"&intervalSection="+key,
                http: $http,
                title: '确定要取消预约吗?',
                text: '取消后所有信息无法恢复',
                confirmButtonText: '确定',
    ...
    $scope.removeDataSite = function (yardId,cardId, con, key) {
        var str = con.about_interval_section;
        var time = con.aboutDate + ' ' + (str.substring(0,5));
        var date = new Date(time);
        var time3 = Date.parse(date);
        if (time3 < Date.parse(new Date())) {
            Message.warning('场地预约时间已开始不能取消预约');
            return;
        }
        Sweety.remove({
    ...
    $scope.siteReservation = function (key,num) {
        ...
        var data = {
            ...
            memberId: $scope.memberId,
            memberCardId: $scope.memberCardId1,
