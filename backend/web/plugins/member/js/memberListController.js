...
angular.module('App').controller('indexCtrl', function ($scope, $http, Upload, $timeout,$interval,$compile) {
    ...
    $scope.updateMemberInVenue = function(memberId)
    {
        $scope.memberId = memberId;
        $('#myModalsVenue').modal('show');
        $http.get('/site/get-auth-venue?memberId='+memberId).success(function (result) {
            $scope.currentVenue = result.venues;
            $scope.memberIds = result.memberId;
        });
    }
    ...
    $('#buyClassLimit').daterangepicker(null, function(start, end, label) {});
    $('#subscriptionDate').daterangepicker(null, function (start, end, label) {
        // console.log(start.toISOString(), end.toISOString(), label);
    });
    $('#updateDepositDate').daterangepicker(null, function (start, end, label) {
        // console.log(start.toISOString(), end.toISOString(), label);
    });
    $('#cardReservation').daterangepicker(null, function(start, end, label) {
    ...
    $('#birthUserLimit').daterangepicker(null, function(start, end, label) {});
    ...
    $scope.path = function () {
        $scope.MAIN = {
            'API': {
                ...
                'memberYardRecord': "/member/get-member-about-yard-record?memberId=" + $scope.id,//场地记录
    ...
    $scope.init = function () {
        ...
        $scope.bindIdCard   = "";
        $scope.bindCode     = "";
        $scope.isDistributionCoach  = '';
        $scope.choosePrivateTeacher   = '';
    ...
    //是否分配私教change事件
    $scope.isCoachChange = function(id){
        $scope.distributionCoachBtn = false;
    }
    /***清空搜索*****/
    ...
    $scope.formatDate = function () {
        ...
        if($("#reservation").val() != ''){
            ...
            $scope.startTime = startTime+' '+ "00:00:00";
    ...
    $scope.cardFormatDate = function () {
        $scope.cardTimeStart = '';
        $scope.cardTimeEnd = '';
        if($("#cardReservation").val() != ''){
            var cardStartTime = $("#cardReservation").val().substr(0, 10);
            $scope.cardTimeStart = cardStartTime+' '+ "00:00:00";
            var cardEndTime = $("#cardReservation").val().substr(-10, 10);
            $scope.cardTimeEnd  = cardEndTime+' '+"23:59:59";
        }

        // $scope.cardTimeStart = $("#cardReservation").val().substr(0, 10) +' ' + "00:00:00";
        // $scope.cardTimeEnd = $("#cardReservation").val().substr(-10, 10) +' ' + "23:59:59";
    };

    $scope.cardFormatDate();
    //购课时间
    $scope.buyClassDateInfo = function(){
        $scope.buyClassTimeStart = '';
        $scope.buyClassTimeEnd = '';
        if($("#buyClassLimit").val() != ''){
            var buyClassStartTime = $("#buyClassLimit").val().substr(0, 10);
            $scope.buyClassTimeStart = buyClassStartTime+' '+ "00:00:00";
            var buyClassEndTime = $("#buyClassLimit").val().substr(-10, 10);
            $scope.buyClassTimeEnd  = buyClassEndTime+' '+"23:59:59";
        }

        // $scope.cardTimeStart = $("#cardReservation").val().substr(0, 10) +' ' + "00:00:00";
        // $scope.cardTimeEnd = $("#cardReservation").val().substr(-10, 10) +' ' + "23:59:59";
    };
    $scope.buyClassDateInfo();
    //分页(朱俊哲注释2018.05.08)
    ...
    //         $.loading.hide();
    //         $('.memberChooseTransfer').children().removeAttr('checked');
    //         //$('.chooseOnePage').removeAttr('checked');
    ...
    $scope.batchDeletingCourseMess = function(){
        ...
        if($scope.allChargeDelArr.length == 0){
    ...
    $(document).on('click', '.memberList', function () {
        $scope.id ='';
        var id = $(this).data('id');
        $scope.changeSellerId = $(this).data('counselor-id');
        $scope.changeCoachId = $(this).data('private-id');
        $scope.selectEntryRecord = '';
        $scope.MemberId123 = id;
        $scope.MemberStatusFlag = status;
        $('div.tab-content .tab-pane').removeClass('active');
    ...
    $(document).on('click', '.chooseOnePage', function () {
        if($(this).is(':checked')) {
            ...
            $('.chooseOnePage').siblings().text('取消本页');
        }else {
            $('.memberChooseTransfer').children().prop('checked','');
            $('.chooseOnePage').siblings().text('选择本页')
        }
  ...
  /*  $scope.otherCheck = function (id) {
        ...
        if($a.is(":checked")){
            $scope.memberIdArr.push(id);
            console.log($scope.memberIdArr)
        }else{
            $scope.memberIdArr.splice($.inArray(id,$scope.otherCheckMemberId),1);
        }
        return false;
    ...
    //获取所有教练
    $scope.getAllPrivateCoach = function () {
        $http.get('/private-teach/private-coach').then(function (response) {
            $scope.allPrivateCoachList = response.data;
        })
    };
    $scope.getAllPrivateCoach();
    ...
    // //上传照片
    $scope.setCover5 = function (file,text) {
        if(file) {
            if(file.size >=2000000){
                swal('上传图片不能大于2M，请重试！', '', 'error');
            }else{
                $scope.setPic5(file,text);
            }
        }
    };
    //发送图片
    $scope.setPic5 = function (file,text) {
        Upload.upload({
            url    : '/class/upload',
            method :'POST',
            ...
            if(results.data.status == 'success'){
                ...
                if (text == 'update') {
                    $scope.MemberData.pic = results.data.imgLink;
                    var data = {
    ...
    $scope.getMemberData = function (id) {
        ...
        $http.get("/user/member-details-card?MemberId=" + id).then(function (result) {
            ...
            $scope.memberAccountId =result.data.member_account_id;
            $scope.countStatus = result.data.count;
            $.loading.hide();
        })
    };
    //添加场地记录列表
    $scope.getYardRecord = function () {
        $http.get($scope.MAIN.API.memberYardRecord).then(function (response) {
            $scope.yardRecordData = response.data.data;
            if ($scope.yardRecordData.length > 0) {
                $scope.memberYardDataList = false;
            } else {
                $scope.memberYardDataList = true;
            }
        });
    };
    //取消预约场地
    ...
    $scope.getMemberInfo = function (id) {
        //$scope.getDetailData(id);
        //获取会员信息资料
        $http.get('/member-card/member-card-status?memberId=' +  $scope.oneMemberInfoId).success(function (response) {
            if (response.status == 'success') {
                $scope.$emit('isMemberCardStatus');
            } else {
                Message.warning(response.data);
            }
        })
    };
    $scope.$on('isMemberCardStatus', function () {
        $scope.getMemberData($scope.oneMemberInfoId)
        $scope.bindModel = '1';
    ...
    $scope.getChangeRecords = function () {
        $.loading.show();
        $http.get("/member/member-change-records?memberCardId=" + $scope.infoId).then(function (data) {
    ...
    $scope.infoChange = function (){
        ...
        $http.get('/member/get-member-card?memberCardId=' + $scope.infoId).success(function (data){
            ...
            if (parseInt($scope.timestamp) > parseInt($scope.cardInfoTime)) {
                $('#daysSpan').text("卡已过期");
    ...
    $(".renewPriceSelect").blur(function (){
        //判断续费价格区间
        if($scope.renewTypeSelect == '1' && $scope.ordinaryRenewal != null && Number($scope.ordinaryRenewal) !=  Number($scope.renewPrice)){
            ...
            $scope.classSurplusPrice = '';
            return
        }else if($scope.maxPrice !=null  && $scope.ordinaryRenewal == null && Number($scope.renewPrice) > Number($scope.maxPrice) ){
            Message.warning("价格不得大于"+ $scope.maxPrice + "元");
            $('.renewPriceSelect').val('');
            return
        }else if( $scope.minPrice !=null && $scope.ordinaryRenewal == null && Number($scope.renewPrice) < Number($scope.minPrice)){
            Message.warning("价格不得小于"+ $scope.minPrice + "元");
            $('.renewPriceSelect').val('');
            return
        }

        if($scope.renewTypeSelect == '2' && $scope.renewEffectiveTimeItem != '' && $scope.renewEffectiveTimeItem != undefined && $scope.renewEffectiveTimeItem != null && $scope.renewTypeSelect == '2'){
   ...
   /* if($scope.renewPrice > $scope.maxPrice){
    ...
    $scope.searchUserClick = function () {
        ...
        var $pattern = /^1((3[0-9]|4[57]|5[0-35-9]|7[0-9]|8[0-9]|9[0-9])\d{8}$)/;
    ...
    $scope.getUpsateCardType = function (data) {
        ...
        if($scope.upSateCardTypeValue == ''){
            if($scope.cardInfo.create_at == null){
                $scope.upsateCardTimeStart = $scope.cardInfo.active_time; //升级卡的开始时间
                $scope.upsateCardEnd = parseInt($scope.cardInfo.active_time) + $scope.upsateDuration; //升级卡的结束时间
            }else{
                $scope.upsateCardTimeStart = $scope.cardInfo.create_at; //升级卡的开始时间
                $scope.upsateCardEnd = parseInt($scope.cardInfo.create_at) + $scope.upsateDuration; //升级卡的结束时间
            }
    ...
    $("#upcardConversionMoneyInput").blur(function (){
        if($scope.upsateCardMoney && $scope.upcardConversionMoney >= 0 && $scope.upcardConversionMoney){
            if (!$scope.getDiscountListValue) {
                var $conversionMoney = parseFloat($scope.upsateCardMoney).toFixed(2) - parseFloat($scope.upcardConversionMoney).toFixed(2);
            } else {
                var $conversionMoney = (parseFloat($scope.upsateCardMoney).toFixed(2) * parseFloat($scope.discountCardList.discount) / 10) - parseFloat($scope.upcardConversionMoney).toFixed(2);
            }
    ...
    $scope.searchCardData = function () {
        ...
        $scope.cardFormatDate();
        $scope.buyClassDateInfo();
        var birthdayStartTime;
        var birthdayEndTime;
        if($('#birthUserLimit').val().indexOf('-') === -1) {
            birthdayStartTime = '';
            birthdayEndTime = '';
        }else {
            birthdayStartTime = $('#birthUserLimit').val().substr(0,10).toString() + ' 00:00:00'
            birthdayEndTime = $('#birthUserLimit').val().substr(13,10).toString() + ' 23:59:59';
        }
        return {
            ...
            buyClassStart : $scope.buyClassTimeStart != undefined ? $scope.buyClassTimeStart : null,
            buyClassEnd: $scope.buyClassTimeEnd != undefined ? $scope.buyClassTimeEnd : null,
            ...
            personalId: $scope.choosePrivateTeach,
            isDistribution: $scope.isDistributionCoach,
    ...
    $scope.searchCard = function () {
        ...
        $scope.initPath();
        $scope.getData();
        $scope.distributionCoachBtn = true;
    };
    /******Enter键搜索*******/
    ...
    //批量分配私教
    $scope.chooseDistributionCoachMemberArr = [];
    $scope.distributionCoach = function () {
        $scope.choosePrivateTeacher = '';
        $scope.chooseDistributionCoachMemberArr = [];
        for(var i = 0; i < $('.memberChooseTransfer').children().length; i++) {
            if($('.memberChooseTransfer').children().eq(i).is(':checked')) {
                $scope.chooseDistributionCoachMemberArr.push($('.memberChooseTransfer').children().eq(i).data('choose'));
            }
        }
        if($scope.chooseDistributionCoachMemberArr.length === 0 || $scope.chooseDistributionCoachMemberArr == null) {
            Message.warning('请先批量选择要分配私教的会员！');
            return false;
        }
        $('#batchDistributionModal').modal('show');
        $scope.getPrivateTeachListData();
        //console.log($scope.chooseDistributionCoachMemberArr);
    };
    //获取私教课程
    $scope.getPrivateTeachListData = function(){
        $http.get('/user/get-hs-course-data').success(function(res){
            //console.log(res)
            $scope.privateTeachListInfo = res.data;
        })
    };
    //选择完会员并选择完私教，发送数据
    $scope.choosePrivateTeachComplete = function () {
        var request = {
            courseId : $scope.choosePrivateClass,//课程id
            memberId : $scope.chooseDistributionCoachMemberArr, //会员id数组
            coachId  : $scope.choosePrivateTeacher               //选择的私教的id
        };
        if($scope.choosePrivateClass == '' || $scope.choosePrivateClass == null || $scope.choosePrivateClass == undefined) {
            Message.warning('请选择课程！');
            return false;
        }
        if($scope.choosePrivateTeacher == '' || $scope.choosePrivateTeacher == null || $scope.choosePrivateTeacher == undefined) {
            Message.warning('请选择教练！');
            return false;
        }
        $http({
            method  : 'post',
            //url     : '/member/update-member-personal',
            url:'/member/batch-allot-private',
            data    : $.param(request),
            headers : {'Content-Type': 'application/x-www-form-urlencoded'}
        }).then(function (response) {
            if(response.data.status.toString() === 'success') {
                Message.success('批量修改成功！');
                $('#batchDistributionModal').modal('hide');
                $scope.initPath();
                $scope.getData();
                $('.memberChooseTransfer').children().prop('checked','');
            }else {
    ...
    //模态框关闭事件
    $('#batchDistributionModal').on('hidden.bs.modal', function (e) {
        // do something...
        $scope.choosePrivateClass = '';
        $scope.choosePrivateTeacher = '';
    })
    ...
    $scope.icCardBinding = function (id) {
        ...
        $http.get('/user/member-details-card?MemberId=' + id).then(function(results){
            //console.log(results);
            $scope.icCardNumberInfo = results.data;
        })
    };
    // IC卡绑定 修改后 点击完成
    $scope.ICCardBindingChange = function () {
        ...
        if($scope.ICCardNumber == null || $scope.ICCardNumber == '' || $scope.ICCardNumber == undefined) {
            Message.warning('绑定的ic卡号不能为空！');
            return;
        }
    ...
    $scope.unbundleBtn = function(id){
        ...
        swal({
            ...
            if (isConfirm) {
                $http.get('/member/del-ic?memberId=' + id).then(function(result){
                    //console.log(result)
                    if(result.data.status == 'success'){
                        //Message.success(result.data.message);
                        swal("", result.data.message, "success");
                        //刷新模态框
                        $scope.icCardBinding( $scope.memberId_bindICCard)
                    }else{
                        Message.warning(result.data.message);
                    }
                })
    ...
    //更换教练
    $scope.changeMemberCoachBtn = function () {
        $http.get('/private-teach/private-coach').then(function (response) {
            $scope.changeMemberCoachList = response.data;
            if(response.data !== null && response.data !== undefined && response.data !== '') {
                $timeout(function () {
                    for(var i = 0;i < $('#changeCoachId').children().length; i++) {
                        if($('#changeCoachId').children().eq(0).text() !== '请选择教练') {
                            $('#changeCoachId').children().eq(0).remove();
                        }
                        if($('#changeCoachId').children().eq(i).val() == $scope.changeCoachId) {
                            $('#changeCoachId').children().eq(i).attr('selected','selected');
                        }
                    }
                },300);
                $timeout(function () {
                    $('#changeMemberCoachModal').modal('show');
                },500)
            }
        });

    };
    $scope.changeMemberCoach = function () {
        $scope.changeCoachId = $('#changeCoachId').find('option:selected').val();
        $http.get('/member/update-private?memberId=' + $scope.MemberId123 + '&privateId=' + $scope.changeCoachId).then(function (response) {
            if(response.data.status == 'success') {
                $scope.initPath();
                $scope.getData();
                Message.success('操作成功！');
                $('#changeMemberCoachModal').modal('hide');
            }else {
    ...
    //获取所有销售
    //更换销售
    $scope.changeMemberSellerBtn = function () {
        $http.get('/user/get-adviser').then(function (response) {
            $scope.changeMemberSellerList = response.data;
            if(response.data !== null && response.data !== undefined && response.data !== '') {
                $timeout(function () {
                    for(var i = 0;i < $('#changeSellerId').children().length; i++) {
                        if($('#changeSellerId').children().eq(i).val() == $scope.changeSellerId) {
                            $('#changeSellerId').children().eq(i).attr('selected','selected');
                        }
                    }
                },300);
                $timeout(function () {
                    $('#changeMemberSellerModal').modal('show');
                },500);
            }
        });
    };
    $scope.changeMemberSeller = function () {
        $scope.changeSellerId = $('#changeSellerId').find('option:selected').val();
        $http.get('/member/update-counselor?memberId=' + $scope.MemberId123 + '&counselorId=' + $scope.changeSellerId).then(function (response) {
            if(response.data.status == 'success') {
                $scope.initPath();
                $scope.getData();
                Message.success('操作成功！');
                $('#changeMemberSellerModal').modal('hide');
            }else {
                Message.warning('操作失败！');
            }
        })
    ...
    $scope.getBindMemberCode = function (mobile) {
        ...
        // 发送验证码
        if ($scope.getMemberCode) {
            $scope.getMemberCode = false
            $http({
    ...
    //请假模态框关闭事件
    $('#myModalsLeave').on('hidden.bs.modal', function (e) {
        $scope.leaveTypeChecked = '';
        $scope.leaveTypeCheckedA = '';
    });
    ...
    //获取折扣
    $scope.getCardDiscount = function () {
        $http.get('/private-lesson/get-charge-discounts').then(function (response) {
            // console.log(response);
            $scope.getBuyCardDiscountList = angular.fromJson(response.data);
            // console.log($scope.getBuyCardDiscountList);
        })
    };

    //选择折扣
    $scope.buyClassDiscountChange = function (discount) {
        $.loading.show();
        if(!$scope.paymentTerm) {
            Message.warning('请先选择付款方式！');
            $scope.PayMoney = '';
            $scope.PayMoney1 = '';
            $scope.getFinalDiscount = '';
            $.loading.hide();
            return ;
        }
        $scope.getFinalDiscount = parseFloat((discount/10).toFixed(2));
        if($scope.getFinalDiscount === 0 || $scope.getFinalDiscount === null || $scope.getFinalDiscount === undefined || $scope.getFinalDiscount === '0') {
            $scope.getFinalDiscount = 1;
        }
        $scope.PayMoney = parseFloat((parseFloat($scope.PayMoney1) * $scope.getFinalDiscount).toFixed(2));
        $.loading.hide();
    };
        ...
        if (parseInt(value) == 2) {
            ...
            if ($scope.allClassDepositMoneyAmount != null) {
                if($scope.AllMoney123 < parseFloat($scope.allClassDepositMoneyAmount)) {
                    ...
                    $scope.PayMoney1 = $scope.AllMoney123 - parseFloat($scope.allClassDepositMoneyAmount);
                }else {
                ...
                $scope.PayMoney1 = $scope.AllMoney123;
            }
        } else {
            // $(".subcriptionB").text($scope.allClassDepositMoneyAmount);
            $scope.PayMoney = $scope.AllMoney123;
    ...
    //选择课程套餐
    $scope.selectPrivateCourseServe = function (id, pic, name, chargeType, total, time) {
        $scope.monthUpNums = '';
        $scope.trues = true;
        $scope.blendId = id;
        if (pic == '') {
    ...
    $scope.getSellSources = function () {
        $http({
            ...
            // console.log($scope.memberSearchData);
        }, function (error) {
            console.log(error);
            Message.error("系统开了会小差,请刷新重试。。。")
        })
    ...
    $scope.getDateStr =  function (validTime) {
        var nowDate = new Date();
        nowDate.setDate(nowDate.getDate()+ validTime);
        var year = nowDate.getFullYear();
        var month = (nowDate.getMonth()+1)<10?"0"+(nowDate.getMonth()+1):(nowDate.getMonth()+1);
        var day = nowDate.getDate()<10?"0"+ nowDate.getDate():nowDate.getDate();
        $scope.monthUpNums = year+"-"+month+"-"+day;
    }
    //计算课程总价
    $scope.courseQuantity = function (value) {
        ...
        if ($scope.chargeType == 'many') {
            if ($scope.valTime) {
                $scope.getDateStr(Number($scope.valTime));
            }
        }
    ...
    $("#completeBuy").click(function () {
        ...
        if (!$scope.privateLessonBuyButtonFlag) {
            ...
            if ($scope.PayMoney == null || $scope.PayMoney == undefined) {
                ...
                $scope.PayMoney1 = $scope.blendMoney;
            }
            if ($scope.paymentTerm == '1') {
                $scope.PayMoney = $scope.blendMoney;
                $scope.PayMoney1 = $scope.blendMoney;
            }
            $scope.buyClassDiscountChange($scope.buyClassDiscount);
            var data123 = {
                ...
                // endTime    : $scope.chargeType == 'many' ? Date.parse($scope.monthUpNums): $scope.monthUpNums,  //结束时间
                endTime    : $scope.chargeType == 'alone' ? $scope.monthUpNums: '',  //结束时间
    ...
    $(function () {
        ...
        //私课升级缴费日期
        $(".privateUpgradeRegisterDate").datetimepicker({
            minView: "month",
            format: 'yyyy-mm-dd',
            language: 'zh-CN',
            autoclose: true,
    ...
    $scope.getLeaveRecord = function (id) {
        $http.get('/member/automatic-leave?memberId=' + $scope.id).then(function (res) {
    ...
    $scope.privateChangeRecord = function () {
        $.loading.show();
        $http.get('/member/personal-change?memberId=' + $scope.MemberId123).then(function (response) {
            if(response.data.data.length === 0) {
    ...
    $scope.SelectMessage = function (value) {
        ...
        if (value == '11') {
            $scope.memberChangeVenue();
        }
    };
    //IC卡绑定记录
    $scope.getIcCardInfo = function(){
        $.loading.show();
        $http.get('/member/ic-bind-records?memberId=' + $scope.MemberId123).then(function (response) {
            if(response.data.data.length === 0) {
    ...
    // 获取场馆变更记录memberChangeVenue()方法
    $scope.memberChangeVenue = function (){
        $http.get("/member/member-change-venue?memberId=" + $scope.MemberId123).success(function (data){
             if (data.length > 0) {
                 $scope.memberChangeVenueList = data;
                 $scope.memberChangeVenueShow = false;
             } else {
                 $scope.memberChangeVenueList = data;
                 $scope.memberChangeVenueShow = true;
             }
        });
    };
    ...
    //修改定金记录
    $scope.depositInfoUpdate = function(object){
        //console.log(object);
        $('#updateDepositModal').modal('show');
        $scope.updateDepositType = object.type;         //定金类型
        $('#updatePrice').val(object.price);            //定金金额
        $scope.updatePayType = object.pay_mode;         //支付方式
        $scope.updateDepositTypeChange(object.type);    //定金类型事件
        $scope.updateSellName = object.sell_people_id;  //售卖人员
        $scope.updateDepositMemberId = object.member_id;//会员id
        $scope.updateDepositId = object.id;             //定金id
        ...
        if($scope.updateDepositType == 1 || $scope.updateDepositType == 3 || $scope.updateDepositType == 4){
            $scope.updateCardTypeStatus = true;
        }else{
            $scope.updateCardTypeStatus = false;
        }
        $('#updateDepositDate').val(timestampToTime(object.start_time) + '- ' + timestampToTime(object.end_time) );//折扣有效期限开始日期
    };
    //定金类型改变事件
    $scope.updateDepositTypeChange = function (value) {
        if(value == 2 || value == 5){
            //获取私教人员信息
            $http.get("/private-teach/private-coach").success(function (response) {
                $scope.saleInfo = response;
            });
            //$scope.updateSellName = '';
            $scope.updateCardTypeStatus = false;
        }else{
            // 获取销售人员信息
            $http.get("/user/get-adviser").success(function (response) {
    ...
    //修改定金记录完成按钮
    $scope.updateDepositBtn = function(){
        var updateDateRange = $("#updateDepositDate").val();
        var startTime = updateDateRange.slice(0, 10)+' 00:00:00';
        var endTime = updateDateRange.slice(13)+' 23:59:59';
        var data = {
            memberId: $scope.updateDepositMemberId,//会员id
            price:  $('#updatePrice').val(),//金额
            startTime: startTime,//开始时间
            endTime: endTime,//结束时间
            payMode: $scope.updatePayType,//付款方式
            payType: $scope.updateDepositType,//定金类型
            seller : $scope.updateSellName,         //销售
            id:$scope.updateDepositId,//定金id
            _csrf_backend: $('#_csrf').val(),
        };
        if ($scope.updateDepositType == '' || $scope.updateDepositType == undefined|| $scope.updateDepositType == null) {
            Message.warning("请选择定金类型");
            return
        }
        ...
        if ($scope.updatePayType == '' || $scope.updatePayType == undefined||$scope.updatePayType == null) {
            Message.warning("请选择付款方式");
            return
        }
        if ($scope.updateSellName == '' || $scope.updateSellName == undefined||$scope.updateSellName == null) {
            Message.warning("请选择销售");
            return
        }
        swal({
    ...
    //修改定金记录模态框关闭事件
    $('#updateDepositModal').on('hidden.bs.modal', function (e) {
        // do something...
        $scope.updateDepositType = ''; //定金类型
        $('#updatePrice').val('');     //定金金额
        $("#updateDepositDate").val(); //有效期限
        $scope.updatePayType = '';     //支付方式
        $scope.updateSellName = '';    //售卖人员
    })
    /**处理搜索到场数据***/
    ...
    $scope.documentTypeChange = function(){
        $scope.MemberDetailsUpdate.id_card = '';
        // $('#exampleInputName2').val('');
    };
    // //身份证验证
    // $scope.testCard = function(){
    //     var reg =/^(\d{15}$|^\d{18}$|^\d{17}(\d|X|x))$/ ;
    //     if(!reg.test($('#exampleInputName2').val())) {
    //         Message.warning("请输入有效身份证");
    //         $scope.MemberDetailsUpdate.id_card = '';
    //         //$('#exampleInputName2').val('');
    //         return false;
    //     }
    ...
    $scope.MemberInfo = function () {
        $http.get('/member/judge-mobile?mobile=' + $scope.MemberDetailsUpdate.mobile+'&memberId='+$scope.MemberDetailsUpdate.id).then(function (response) {
            if(response.data.status == 'error') {
                ...
                //     return;
                // }
                if($scope.MemberDetailsUpdate.name == '' || $scope.MemberDetailsUpdate.name == undefined){
                    Message.warning("姓名不能为空");
                    return false;
                }
                if($scope.MemberDetailsUpdate.document_type === 1){
                    if($scope.MemberDetailsUpdate.id_card != undefined && $scope.MemberDetailsUpdate.id_card != null && $scope.MemberDetailsUpdate.id_card != ''){
                        var reg =/^(\d{15}$|^\d{18}$|^\d{17}(\d|X|x))$/ ;
                        if(!reg.test($scope.MemberDetailsUpdate.id_card)){
                            Message.warning("请输入有效身份证");
                            return false;
                        }
                    }
    ...
    //私课升级
    $scope.upgradePrivateLesson = function (id) {
        $.loading.show();
        //初始化数输入框据
        $scope.privateClassUpgradePrice     = '';
        $scope.privateClassUpgradeName      = '';
        $scope.privateUpgradeClassNum       = '';
        $scope.privateUpgradeCostPrice      = '';
        $scope.privateUpgradePayType        = '';
        $scope.privateUpgradeDeposit        = '';
        $scope.privateUpgradeRegisterDate   = '';
        $scope.privateUpgradeCoach          = '';
        $scope.privateUpgradeCoachChannels  = '';
        $scope.privateUpgradeGetGift        = '';
        $scope.privateUpgradeNote           = '';
        $scope.upgradeClassMoney            = '';
        $scope.catchUpgradeClassMoney       = '';
        $scope.allUpgradeClassMoney         = '';
        $scope.upgradeMonth                 = '';
        ...
        $scope.getToday();
        $('.privateUpgradeRegisterDate').val($scope.dateInput);
        if($scope.upgradeClassMoney === '') {
            $scope.upgradeClassMoney = 0;
        }
        if($scope.catchUpgradeClassMoney === '') {
            $scope.catchUpgradeClassMoney = 0;
        }
        if($scope.upgradeMonth === '') {
            $scope.upgradeMonth = 0;
        }
        //选择定金默认是disabled的
        $scope.privateUpgradePayTypeBool = true;
        //完成按钮默认是非disabled的
        $scope.catchUpgradeClassMoneyBool = false;
        $('.addUpgradePayBox').children().eq(0).children().find('select').val('');
        $('.addUpgradePayBox').children().eq(0).children().find('input[name=privateUpgradePayNum]').val('');
        $('.addUpgradePayBox').children().eq(0).siblings().remove();
        // $('.privateUpgradeRegisterDate').val('');
        $scope.oldPrivateClassId            = id;
        $scope.privateClassUpgradeId        = null;
        var leftMoney = (parseFloat($scope.privateLessonTemplet.money_amount) / parseFloat($scope.privateLessonTemplet.course_amount)) * parseFloat($scope.privateLessonTemplet.overage_section);
        $scope.nowSurplusMoney = leftMoney.toFixed(2);
        //获取私课升级定金
        $http.get('/member/deposit-data-type?memberId=' + $scope.id + '&type=5').then(function (response) {
            $scope.upgradePrivateLessonDeposit = angular.fromJson(response.data.type);
            var oldDeposit = response.data.allVoucher == null ? 0 : response.data.allVoucher;
            var newDeposit = response.data.allPrice == null ? 0 : response.data.allPrice;
            $scope.upgradePrivateDepositSurplus = oldDeposit + newDeposit;
            $scope.upgradePrivateDepositSurplus1 = oldDeposit + newDeposit;
        });
        //获取私教教练列表
        $http.get('/private-teach/private-coach').then(function(data) {
            $scope.upgradePrivateCoachList = data.data;
        });
        //获取销售渠道
        $scope.getSellSources();
        $.loading.hide();
        $('#upgradePrivateLessonModal').modal('show');
    };

    //选择私教产品点击遍历
    $scope.choosePrivateProductUpgrade = function () {
        $.loading.show();
        $http({
            method: "GET",
            url: '/private-teach/get-private-teach-all?memberId=' + $scope.id
        }).then(function (data) {
            if (data.data.data.alone != 0) {
                $scope.aloneLessonData = data.data.data.alone;
                $scope.manyLessonData = data.data.data.many;
            }
            $.loading.hide();
        });
        $('#privateProductUpgradeModal').modal('show');
    };
    //选择私教产品完成
    $scope.completePrivateClassUpgrade = function (price, id, name, type, month) {
        if(id !== undefined && id !== null && id !== '') {
            if($scope.privateClassUpgradeId == $scope.oldPrivateClassId) {
                Message.warning('选择的升级私课不能与原私课相同！')
                return false;
            }
            $scope.privateUpgradeClassNum       = '';
            $scope.privateUpgradeCostPrice      = '';
            ...
            $scope.allUpgradeClassMoney         = '';
            $scope.upgradeMonth                 = '';
            $scope.privateClassUpgradePrice     = price;
            $scope.privateClassUpgradeId        = id;
            $scope.privateClassUpgradeName      = name;
            if(parseInt(type) === 1) {
                $scope.privateClassUpgradeType = 'many';
            }else {
                $scope.privateClassUpgradeType = 'alone';
            }
            $scope.privateClassUpgradeMonth     = month;
            //选择定金默认是disabled的
            $scope.privateUpgradePayTypeBool = true;
            //完成按钮默认是非disabled的
            $scope.catchUpgradeClassMoneyBool = false;
            ...
            $('.addUpgradePayBox').children().eq(0).siblings().remove();
            // $('.privateUpgradeRegisterDate').val('');
            $('#privateProductUpgradeModal').modal('hide');
        }else {
            Message.warning('选择失败！请刷新后重试！');
            return false;
        }
    };

    //课程数量输入框change事件  计算金额
    $scope.privateUpgradeClassNumBlur = function () {
        if(!$scope.privateClassUpgradeId) {
            Message.warning('请先选择私教产品！');
            return;
        }
        if($scope.privateUpgradeClassNum === '' || $scope.privateUpgradeClassNum === undefined || $scope.privateUpgradeClassNum === null) {
            $scope.privateUpgradeClassNum = 0;
        }
        $scope.privateUpgradeCostPrice = $scope.privateUpgradeClassNum * parseInt($scope.privateClassUpgradePrice);
        $scope.upgradeClassMoney = $scope.privateUpgradeClassNum * parseInt($scope.privateClassUpgradePrice);
        $scope.upgradeClassMoney1 = $scope.privateUpgradeClassNum * parseInt($scope.privateClassUpgradePrice);
        $scope.choosePrivateUpgradeDeposit();
        $scope.payClassMoney = $scope.upgradeClassMoney - $scope.nowSurplusMoney;
        $scope.catchUpgradeClassMoney = $scope.payClassMoney.toFixed(2);
        if($scope.catchUpgradeClassMoney < 0) {
            $scope.catchUpgradeClassMoneyBool = true;
        }else {
            $scope.catchUpgradeClassMoneyBool = false;
        }
        if($scope.privateUpgradeClassNum !== '') {
            $scope.upgradeMonth = Math.ceil($scope.privateUpgradeClassNum / $scope.privateClassUpgradeMonth)
        }
    };

    //付款方式change事件
    $scope.privateUpgradePayTypeChange = function (type) {
        type.toString() === '2' ? $scope.privateUpgradePayTypeBool = false : $scope.privateUpgradePayTypeBool = true;
    };

    //定金选择change事件
    $scope.choosePrivateUpgradeDeposit = function () {
        var num = 0;
        for(var i = 0; i < $('.privateUpgradeDeposit').find('option:selected').length; i++) {
            num = num + parseInt($('.privateUpgradeDeposit').find('option:selected').eq(i).data('price'));
        }
        $scope.upgradeClassMoney = $scope.upgradeClassMoney1 - num;
        $scope.upgradePrivateDepositSurplus = $scope.upgradePrivateDepositSurplus1 - num;
        $scope.catchUpgradeClassMoney = $scope.upgradeClassMoney - $scope.nowSurplusMoney;
        if($scope.catchUpgradeClassMoney < 0) {
            $scope.catchUpgradeClassMoneyBool = true;
        }else {
            $scope.catchUpgradeClassMoneyBool = false;
    ...
    //付款途径下拉框change事件
    $scope.upgradeMoneyTypeChange = function () {
        $scope.upgradeClassTypeArr = [];
        for(var j = 0; j < $('.addUpgradePayChild').children().find('.privateUpgradePayWay').length; j++) {
            if(!$('.addUpgradePayChild').children().find('.privateUpgradePayWay').eq(j).val()) {
                Message.warning('请选择付款途径！');
                return;
            }
            $scope.upgradeClassTypeArr.push($('.addUpgradePayChild').children().find('.privateUpgradePayWay').eq(j).val())
        }
        if($scope.isRepeat($scope.upgradeClassTypeArr)) {
            Message.warning('付款途径不能重复！');
            return false;
        }
    };

    //多种付款途径输入的change事件
    $scope.upgradeMoneyInputChange = function (type) {
        var forNum = 0;
        $scope.upgradeClassMoneyArr = [];
        if(type === 'delete') {
            $timeout(function () {
                for(var i = 0; i < $('.addUpgradePayChild').children().find('.privateUpgradePayNum').length; i++) {
                    forNum = parseFloat($('.addUpgradePayChild').children().find('.privateUpgradePayNum').eq(i).val());
                    if(!$('.addUpgradePayChild').children().find('.privateUpgradePayNum').eq(i).val()) {
                        forNum = 0;
                    }
                    $scope.upgradeClassMoneyArr.push(forNum);
                }
                $scope.allUpgradeClassMoney = $scope.isCompare($scope.upgradeClassMoneyArr);
                $.loading.hide();
            },300);
        }else {
            for(var i = 0; i < $('.addUpgradePayChild').children().find('.privateUpgradePayNum').length; i++) {
    ...
    //删除按钮点击事件
    $scope.upgradeMoneyDelClick = function () {
        $.loading.show();
        $scope.upgradeMoneyInputChange('delete');
        $scope.upgradeMoneyTypeChange();
    };
    $scope.privateUpgradeCostPriceClick = function () {
        if($scope.privateUpgradeClassNum) {
            Message.warning('请先选择课程数量!')
        }
    };
    //升级私课完成按钮
    var upgradeClassData;
    $scope.upgradeClassPayMethodArr = [];
    $scope.completeUpgrade = function () {
        ...
        if(!$scope.privateUpgradeClassNum) {
            Message.warning('请选择课程数量！');
            return;
        }
        if(!$scope.privateUpgradePayType) {
            Message.warning('请选择付款方式！');
            return;
        }
        if($scope.privateUpgradePayType.toString() === '2' && $scope.privateUpgradeDeposit == null) {
            Message.warning('您选择了定金付款！请选择要使用的定金！');
            return;
        }
        if($scope.catchUpgradeClassMoney < 0) {
            Message.warning('补交金额小于升级最低限额！');
            $scope.catchUpgradeClassMoneyBool = true;
            return;
        }else {
            $scope.catchUpgradeClassMoneyBool = false;
        }
        if(!$('.privateUpgradeRegisterDate').val()) {
            Message.warning('请选择缴费日期！');
            return;
        }
        if(!$scope.privateUpgradeCoach) {
            Message.warning('请选择销售的私教！');
            return;
        }
        if(!$scope.privateUpgradeCoachChannels) {
            Message.warning('请选择私教的渠道！');
            return;
        }
        if(!$scope.upgradeMoneyType){
            Message.warning('请选择付款途径！');
            return false;
        }
        if(!$scope.privateUpgradeGetGift) {
            Message.warning('请选择是否已领取赠品！');
            return;
        }
        $scope.upgradeMoneyTypeChange();
        $scope.upgradeMoneyInputChange();
        for(var i = 0; i < $('.addUpgradePayChild').children().find('.privateUpgradePayNum').length; i++) {;
            if(!$('.addUpgradePayChild').children().find('.privateUpgradePayNum').eq(i).val()) {
                Message.warning('请填写所有付款金额！');
                return false;
            }
        }
        if($scope.isRepeat($scope.upgradeClassTypeArr)) {
        ...
        if(parseFloat($scope.allUpgradeClassMoney) !== parseFloat($scope.catchUpgradeClassMoney)) {
            Message.warning('付款总额应等于补交金额！');
            return false;
        }
        for(var i = 0; i < $('.addUpgradePayBox').children().length; i++) {
            upgradeClassData = {
                type: $('.addUpgradePayBox').children().eq(i).children().find('select').val(),
                price: $('.addUpgradePayBox').children().eq(i).children().find('input[name=privateUpgradePayNum]').val()
            };
            $scope.upgradeClassPayMethodArr.push(upgradeClassData);
        }
        var request = {
            memberId        : $scope.id,                                //会员ID
            chargeId        : $scope.privateClassUpgradeId,             //私教课程ID
            chargeType      : $scope.privateClassUpgradeType,           //私课类型
            coachId         : $scope.privateUpgradeCoach,               //私教销售ID
            nodeNumber      : $scope.privateUpgradeClassNum,            //课程节数（套餐数量）
            saleType        : $scope.privateUpgradeCoachChannels,       //销售渠道  (存到了业务备注字段中)
            renewTime       : $('.privateUpgradeRegisterDate').val(),   //缴费日期
            payType         : $scope.privateUpgradePayType,             //收款方式
            payMethod       : $scope.upgradeClassPayMethodArr,          //付款途径
            depositArrId    : $scope.privateUpgradeDeposit,             //使用定金的id数组
            giftStatus      : $scope.privateUpgradeGetGift,             //赠品是否被领取
            totalPrice      : $scope.upgradeClassMoney1,                //买课总价（新加）
            netPrice        : $scope.catchUpgradeClassMoney,            //实收价格
            cashCoupon      : 0,                                        //代金券
            deposit         : $scope.upgradePrivateDepositSurplus1,     //定金
            endTime         : $scope.upgradeMonth,                      //自定义变量，课程有效期
            buyNote         : $scope.privateUpgradeNote,                //私课购买备注
            oldChargeId     : $scope.privateLessonTemplet.id            //老私教课程ID
        };
        $http({
            method: 'POST',
            url: '/member/private-teaching-upgrade',
            data: $.param(request),
            headers: {'Content-Type': 'application/x-www-form-urlencoded'}
        }).then(function (response) {
            if(response.data.status.toString() === 'success') {
                Message.success(response.data.data);
                $('#upgradePrivateLessonModal').modal('hide');
                $scope.privateEducationPurchaseInit($scope.id);
            }else {
                Message.warning('升级失败！请刷新重试！');
                return false;
            }
        })
    };
    ...
    //私课查看会员协议按钮
    $scope.contractInfoBtn = function(id){
        //console.log(id)
        $('#contractInfoModal').modal('show');
        if(id == null || id == ''){
            $scope.contentTeach = '';
        }else{
            $http.get('/contract/get-deal-change?id='+id).then(function (result) {
                console.log(result.data)
    ...
    /********上传私教合同事件************/
    //上传私教合同按钮
    $scope.uploadContract = function(memberId,id,type){
        $('#uploadContract').modal('show');
        $scope.contractMemberId = memberId;
        $scope.orderDetailsId = id;
        $scope.uploadContractType = type;
        $scope.getContractImgData();
    };
    //获取合同照片详情
    $scope.getContractImgParam = function(){
        return {
            type : $scope.uploadContractType,
            typeId : $scope.orderDetailsId,
            memberId : $scope.contractMemberId,
        }
    };
    //获取老合同照片
    $scope.getContractImgData = function(){
        $http.get('/member-card/get-deal-pic?' + $.param($scope.getContractImgParam())).success(function(result){
            if(result.pic == null){
                $scope.contractOldImgArr = [];
                $scope.uploadPic = false;
            }else{
                $scope.contractOldImgArr = result.pic;
                $scope.uploadPic = true;
            }
        });
    };
    $scope.showOldContractDel = function(index){
        $('.deleteOldContractImg').eq(index).css('display','block');
    };
    $scope.hideOldContractDel = function(index){
        $('.deleteOldContractImg').eq(index).css('display','none');
    };
    //删除照片
    $scope.delOldContractImg = function(index){
        $scope.contractOldImgArr.splice(index,1);
        if($scope.contractOldImgArr.length == 1 && $scope.contractOldImgArr[0] == ''){
            $scope.contractOldImgArr = [];
        }
    };

    // //上传照片
    $scope.setCover10 = function (file) {
        if(file) {
            if(file.size >=2000000){
                swal('上传图片不能大于2M，请重试！', '', 'error');
            }else{
    ...
    //发送图片
    $scope.contractImgArr = [];
    $scope.setPic10 = function (file) {
        Upload.upload({
            url    : '/class/upload',
            method :'POST',
            data   : {UploadForm: {imageFile: file}, _csrf_backend: $('#_csrf').val()},
            ...
            if(results.data.status == 'success'){
                $scope.contractImgArr.push(results.data.imgLink);
            }
        });
    };
    $scope.showContractDel = function(index){
        $('.deleteContractImg').eq(index).css('display','block');
    };
    $scope.hideContractDel = function(index){
        $('.deleteContractImg').eq(index).css('display','none');
    };
    //删除照片
    $scope.delContractImgBtn = function(index){
        $scope.contractImgArr.splice(index,1);
    };
    //查看大图
    $scope.bigImg = function(index,src){
    ...
    $scope.getAll1 = function(){
        $scope.contractAll =  $scope.contractImgArr.concat($scope.contractOldImgArr);
    };
    //保存按钮
    $scope.uploadSuccess = function(){
        $scope.getAll1();
        // if($scope.contractAll.length == 0){
        ...
        var privateParam = {
            ...
            typeId : $scope.orderDetailsId,
            memberId : $scope.contractMemberId,
            pic : $scope.contractAll
        };
        $.loading.show();
        $http({
            url : '/member-card/upload-deal-pic',
            method : 'POST',
            data : $.param(privateParam),
            headers : { 'Content-Type': 'application/x-www-form-urlencoded'}
        }).then(function(result){
            if(result.data.status == 'success'){
                Message.success(result.data.data);
    ...
    $('#uploadContract').on('hidden.bs.modal', function (e) {
        // do something...
        $scope.contractImgArr = [];
    });
    /*******上传合同照片事件结束*********/
    ...
    $scope.addUpgradeClassMethod = function() {
        $scope.attr = 'addUpgradeClass';
        $scope.num = $scope.num + 1;
        $http({
            url: "/member/add-venue",
            data: $.param({
            ...
            $scope.addUpgradeClassHtml = result.data.html;
        })
    };
    $scope.addBuyCardMethod();
    $scope.addBuyClassMethod();
    $scope.addUpgradeClassMethod();
    //完成时获取新的付款途径
    ...
    $scope.unfreeze = function(){
        //console.log( $scope.memberAccountId)
        swal({
            title: "确定解冻吗？",
            text: "此操作会解冻当前账户！",
            type: "warning",
            ...
            if (isConfirm) {
                if($scope.memberAccountId != null){
                    $http({
                        url: '/user/update-member-count?memberAccountId=' + $scope.memberAccountId,
                        method: 'POST',
                        headers: {'Content-Type': 'application/x-www-form-urlencoded'}
                    }).then(function (data){
                        console.log(data)
