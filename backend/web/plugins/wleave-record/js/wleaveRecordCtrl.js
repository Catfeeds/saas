...
angular.module('App').controller('wLeaveRecordCtrl',function($scope,$http,$timeout){
    ...
    //批量同意按钮
    $scope.batchReviewBtn = function () {
        $scope.agreeMessageIdArr = [];  //请假信息id
        for(var i = 0; i < $('.memberChooseTransfer').children().length; i++) {
            if($('.memberChooseTransfer').children().eq(i).is(':checked')) {
                $scope.agreeMessageIdArr.push($('.memberChooseTransfer').children().eq(i).data('choose'));
            }
        }
        if($scope.agreeMessageIdArr.length === 0 || $scope.agreeMessageIdArr == null) {
            Message.warning('请先选择要批量同意请假的会员！');
            return false;
        }
        $('#batchReviewModal').modal('show');
        $scope.leaveMemberNum = $scope.agreeMessageIdArr.length;
        //批量同意url
        $scope.batchUrl = '/special-leave/batch-update-status?' + $.param($scope.batchParam());
    };
    //参数
    $scope.batchParam = function(){
        return{
            idArr : $scope.agreeMessageIdArr
        }
    };
    //模态框完成按钮
    $scope.agreeBtn = function(){
        $.loading.show();
        $http.get($scope.batchUrl).success(function(result){
            //console.log(result);
