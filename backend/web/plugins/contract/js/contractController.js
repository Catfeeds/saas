angular.module('App').controller('contractCtrl',function($scope,$http){
   ...
   $scope.getDealSaveData = function () {
    ...
    //获取合同详情
    // $scope.contractFun = function () {
    //     $http.get('/contract/change-deal-record?dealId='+$scope.listId).then(function (result) {
    //         $scope.contractNames = result.data.deal;
    //     })
    // }
    $scope.contractChange = function (index) {
        $scope.contractMessage = $scope.contractNames[Number(index)];
        $('#contractContentDetail').html($scope.contractMessage.intro);
    };
    //获取合同详情
    $scope.contractDetail = function (id) {
        $scope.contractId = '0';
        $http.get('/contract/change-deal-record?dealId='+ id).then(function (result) {
            $scope.contractNames = result.data.deal;
            $scope.contractMessage = $scope.contractNames[0];
            $('#contractContentDetail').html($scope.contractNames[0].intro);
        })
        // $http.get('/contract/get-deal-one?id='+id).then(function (result) {
        //     $scope.dealData = result.data.data;
