...
angular.module('App').controller('storeCtrl',function($scope,$http,$timeout){
    ...
    $scope.getAllVenue = function(){
        ...
        $http.get('/site/get-auth-venue').then(function (result) {
            if(result.data != undefined && result.data != ""){
                $scope.allVenueLists      = result.data;
            }else{
                $scope.allVenueLists = '暂无数据';
            }
        });
    };
    /**
    ...
    $scope.venueChange = function(id){
        //console.log($scope.homePageWarehouseVenue)
        if(id != ''){
            //获取部门
            $http.get('/personnel/get-department?depId=' + id).then(function (result) {
                //console.log(result)
   ...
   /*------- 1、新增仓库功能Start--------*/
    ...
    $scope.addClassroomComplete = function(){
        ...
        if($scope.addWarehouseVenue == undefined ||  $scope.addWarehouseVenue == '' ||  $scope.addWarehouseVenue == null ){
    ...
    $scope.getWarehouseSearchData = function(){
       var homeDate = $('#storeDate').val();
        ...
        return {
            ...
            department_id   :$scope.indexDepartmentId != undefined && $scope.indexDepartmentId != ''? $scope.indexDepartmentId :null,
    ...
    $scope.getDepartmentData = function(){
        $http.get('/personnel/get-department-all-data').then(function (result) {
            if(result.data.department != undefined && result.data.department != ""){
                $scope.departmentList = result.data.department;
            }else{
                $scope.departments = [];
            }
        });
    ...
    $scope.getAllAddShopData = function(){
        return {
            ...
            department_id    :$scope.departmentId != undefined && $scope.departmentId != ''? $scope.departmentId :null,//部门
    ...
    $scope.enterStorageComplete = function(){
        ...
        if($scope.enterStorageMoney== undefined || $scope.enterStorageMoney == ''|| $scope.enterStorageMoney ==null){
        ...
        $http({
            ...
            if(response.data.status == "success"){
                ...
                //刷新模态框
                if($scope.shopDetialObject != undefined && $scope.shopDetialObject != '' && $scope.shopDetialObject != null){
                    $scope.shopDetailClick($scope.shopDetialObject)
                }
    ...
    $('#enterStorageModal').on('hidden.bs.modal', function (e) {
        // do something...
    ...
    $('#shopDetailModal').on('hidden.bs.modal', function (e) {
        // do something...
        $scope.shopDetialObject = '';
    });
    /*----------入库End------------*/
    ...
    $scope.changeGoodsInfo = function(id,venueId){
        ...
        $http.get('/personnel/get-department-all-data').then(function (result) {
            $scope.departmentListData = result.data.department;
        });
        $scope.goodId = id;
    ...
    $scope.done = function(id){
        var goodsInfo = {
            goodsName      : $scope.goodsInfo.goods_name,       //商品名称
            storeId        : $scope.goodsInfo.store_id,         //仓库名称
            venueId        : $scope.goodsInfo.venue_id,         //所属场馆
            department_id  : $scope.goodsInfo.department_id,    //部门名称
            goodsAttribute : $scope.goodsInfo.goods_attribute,  //商品类型
            goodsTypeId    : $scope.goodsInfo.goods_type_id,    //商品品类
            brand          : $scope.goodsInfo.goods_brand,      //商品品牌
            model          : $scope.goodsInfo.goods_model,      //商品型号
            producer       : $scope.goodsInfo.goods_producer,   //生产商
            supplier       : $scope.goodsInfo.goods_supplier    //供应商
        };
        if($scope.goodsInfo.goods_name == undefined || $scope.goodsInfo.goods_name == '' || $scope.goodsInfo.goods_name == null){
            Message.warning('请选择商品名称');
            return;
        }
        if($scope.goodsInfo.store_id == undefined || $scope.goodsInfo.store_id == ''|| $scope.goodsInfo.store_id ==null){
            Message.warning('请选择仓库');
            return;
        }
        if($scope.goodsInfo.venue_id == undefined || $scope.goodsInfo.venue_id == ''|| $scope.goodsInfo.venue_id ==null){
            Message.warning('请选择场馆');
            return;
        }
        if($scope.goodsInfo.department_id == undefined || $scope.goodsInfo.department_id == ''|| $scope.goodsInfo.department_id ==null){
            Message.warning('请选择部门');
            return;
        }
        if($scope.goodsInfo.goods_attribute == undefined || $scope.goodsInfo.goods_attribute == ''|| $scope.goodsInfo.goods_attribute ==null){
            Message.warning('请选择商品类型');
            return;
        }
        if($scope.goodsInfo.goods_type_id == undefined || $scope.goodsInfo.goods_type_id == ''|| $scope.goodsInfo.goods_type_id ==null){
            Message.warning('请选择商品品类');
            return;
        }
        $http({
            ...
            if(response.data.status == "success"){
                $scope.WarehouseKeywordsSearch($scope.goodId);
                $scope.getWarehouseStockAndMoney($scope.goodId);
                $scope.getShopMessage($scope.goodId);
                $scope.getShopDetailLists($scope.goodId);
   ...
   //删除商品类型
    ...
    $scope.outerStorageComplete = function(){
        var outerStorageData  ={
            _csrf_backend : $('#_csrf').val(),
            goodsId       : $scope.warehouseShopId,
            number        : $scope.outerStorageNum != undefined && $scope.outerStorageNum != '' ? $scope.outerStorageNum : null,   //出库数量
            note          : $scope.exitStorageNote != undefined && $scope.exitStorageNote != '' ? $scope.exitStorageNote : null    //出库备注
        };
    ...
    $scope.allocationOrderComplete = function(){
        ...
        if($scope.allocationOrderNewGoodsId == null || $scope.allocationOrderNewGoodsId == '' || $scope.allocationOrderNewGoodsId == undefined){
            Message.warning('被申请方找不到这个商品!');
            return;
        }
