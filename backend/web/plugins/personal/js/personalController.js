var app = angular.module('App').controller('personalCtrl', function($scope,$http,Upload, $sce){
    ...
    $scope.rr = $sce.trustAsResourceUrl;
    //展示
    $scope.uploadPhoto = function (id) {
        ...
        $scope.videoUrl = '' ;
        $scope.isActive = true;
        $scope.uploadType = 'pic';
        $scope.getVideoAndPics(id);
        $('#fieldModals').modal('show');
    }
    $scope.isActive = true;
    // 切换图片还是视频
    $scope.changeUploadType = function (type) {
    ...
    $scope.setCover5 = function (file,text) {
        if(file) {
            if ($scope.uploadType == 'pic') {
                if (file.type.indexOf('image') == -1) {
                    Message.warning('上传不是图片类型');
                    return;
                }
                ...
                if (file.type.indexOf('video') == -1) {
                    Message.warning('上传不是视频类型');
                    return;
                }
                if(file.size >=20000000){
                    swal('上传视频不能大于20M，请重试！', '', 'error');
                }else{
                    $scope.setVideo5(file,text) ;
                }
            }
        }
    };
    //发送视频
    $scope.setVideo5 = function (file,text) {
        // if(file.type.join('/').indexOf('img') )
        if(file.size >=20000000){
            swal('上传视频不能大于20M，请重试！', '', 'error');
            return ;
        }
        ...
        Upload.upload({
            ...
            console.log(results.data.imgLink);
            $scope.isLoading = false;
            $scope.videoUrl =  results.data.imgLink ;
        });
    };
    //发送图片
    $scope.setPic5 = function (file,text) {
        ...
        Upload.upload({
            ...
            $scope.picUrl.push(results.data.imgLink);
        });
    };

    $scope.getArray = function () {
        if ($scope.uploadType == 'pic') {
            if (!$scope.picUrl.length && !$scope.picsAll3.length) {
                Message.warning('请上传照片');
                return;
            }
            $http.post('/personnel/personality-picture',
                {url: $scope.picUrl, type: 1, employee_id: $scope.personalId}).success(function (response) {
            ...
            $http.post('/personnel/personality-video',
                {url: $scope.videoUrl, type: 2, employee_id: $scope.personalId}).success(function (response) {
                // console.log(response);
                if(response.status == 'success'){
                    Message.success(response.data);
                }else{
        ...
        $('#fieldModals').modal('hide');
    }
    $scope.delVideo = function () {
        if ($scope.videoId) {
            $http.get('/personnel/personality-del?id=' + $scope.videoId).success(function (response) {
                $scope.getVideoAndPics($scope.personalId);
    ...
    $scope.delPic = function (index, isUpload) {
        if (isUpload == 'yesUpload') {
            $http.get('/personnel/personality-del?id=' + index).success(function (response) {
                $scope.getVideoAndPics($scope.personalId);
                Message.success('删除照片成功');
            })
        } else {
            $scope.picUrl.splice(index, 1);
            Message.success('删除照片成功') ;
        }
    }

    $scope.getVideoAndPics = function (id) {
        $scope.Id = id
        $http.get('/personnel/personality-details?employeeId=' + id +'&type=1').success(function (response) {
            $scope.picsAll3 = response;
            $scope.$emit('isFinish') ;
        })
    }
    $scope.$on('isFinish', function () {
        $http.get('/personnel/personality-details?employeeId=' + $scope.Id +'&type=2').success(function (response) {
            $scope.videoUrl1 = response;
            $scope.videoId = $scope.videoUrl1.id
            $scope.videoUrl = $scope.videoUrl1.url;
            $scope.picsAll = $scope.picsAll3 ;
        })
    })
    //取消潜在会员约课
    ...
    $scope.SelectTypeMessage = function(selectEntryRecord){
        if(selectEntryRecord == '' || selectEntryRecord == undefined){
            // alert($scope.employeeDetailsId);
            $.loading.show();
            $scope.recordListData = '';
            $http.get('/personnel/turn-member-info?employeeId=' + $scope.employeeDetailsId).then(function(result){
                //console.log(result);
                if(result.data.data.length != 0){
                    $scope.recordListData = result.data.data;
                    $scope.recordCardNoData = false;
                }else{
                    $scope.recordListData = result.data.data
                    $scope.recordCardNoData = true;
                }
                $.loading.hide();
            })
        }else if(selectEntryRecord == 1){
            $.loading.show();
            $scope.employeeAboutClassListData = '';
            $http.get('/personnel/employee-about-class?employeeId=' + $scope.employeeDetailsId).then(function(result){
                if(result.data.data.length != 0){
                    $scope.employeeAboutClassListData = result.data.data;
                    $scope.employeeAboutPages = result.data.pages;
                    $scope.employeeAboutClassNoData = false;
                }else{
                    $scope.employeeAboutClassListData = result.data.data;
                    $scope.employeeAboutClassNoData = true;
                }
                $.loading.hide();
            })
        }
    ...
    /****** 打印机start *****/
    //打印单条
    $scope.employeeAboutPrints = function (item) {
        $scope.printers = item;
        // console.log(item);
        var  urls = '/check-card/get-about-class-detail?id='+ item.id;
        $http({
            method: 'GET',
            url:urls,
        }).then(function (success) {
            $scope.aboutPrints(item);
        });
    };
    ...
    $scope.aboutPrints = function (item) {
        ...
        //
        if(timestamp > item.groupClass.end * 1000){
            Message.warning('该课程已结束!不能打印小票');
            return false;
        }
        $scope.printTicket();
    };
    ...
    $scope.updateAboutStatus = function () {
        $http.get('/check-card/update-about-print?id='+$scope.aboutId+'&type=one').then(function (result) {
            $scope.SelectTypeMessage(1);
        });
    };
    /****** 打印机end ******/

    $scope.newTime = parseInt(Date.parse(new Date())/1000);
    //取消员工约课
    $scope.emplyeeCancelAppointment = function(id,startTime){

        var timestamp = Date.parse(new Date());
        if(timestamp > startTime * 1000){
            Message.warning('该课程已开始,不能取消！');
            return false;
        }
        Sweety.remove({
            url              : "/check-card/cancel-about-class?id="+id,
    ...
    $scope.departmentChange = function (id) {
        ...
        var sendCompany = ($scope.sentCompanyId === undefined || $scope.sentCompanyId === '') ? null : $scope.sentCompanyId;
        $http({method:'get',url:'/personnel/get-employee-position?branchId='+id+'&companyId=' + sendCompany}).then(function (data) {
    ...
    $scope.customAddOk = function (data) {
        ...
        var datas ={
            ...
            companyId : ($scope.sentCompanyId === undefined || $scope.sentCompanyId === '') ? null : $scope.sentCompanyId,
            venueId : ($scope.sentVenueId === undefined || $scope.sentVenueId === '') ? null : $scope.sentVenueId,
        ...
        $http({method:'post',url:'/personnel/set-employee-position',data:$.param(datas),headers: {'Content-Type': 'application/x-www-form-urlencoded'}}).then(function (success) {
            ...
            $http({method:'get',url:'/personnel/get-employee-position?branchId='+$scope.department+'&companyId='+datas.companyId}).then(function (data) {
                $scope.getEmployeePositionData = data.data.attributes;
            },function (error) {console.log(error);Message.error("系统开了会小差,请刷新重试。。。")})
            $("#sellSource1").hide();
    ...
    $scope.removeCustom = function () {
        ...
        var companyId = ($scope.sentCompanyId === undefined || $scope.sentCompanyId === '') ? null : $scope.sentCompanyId;
        if (id != undefined) {
            Sweety.remove({
                ...
                $http({method:'get',url:'/personnel/get-employee-position?branchId='+$scope.department+'&companyId='+companyId}).then(function (data) {
                    $scope.getEmployeePositionData = data.data.attributes;
                },function (error) {console.log(error);Message.error("系统开了会小差,请刷新重试。。。")})
                $scope.position = '';
            },function(){
    ...
    $scope.add = function() {
        $scope.addData = function () {
            return {
                ...
                work_url        : $scope.work_url       != undefined && $scope.work_url       != "" ?$scope.work_url       : null,//作品地址
                intro        : $('.note-editable').text().trim()       != undefined && $('.note-editable').text().trim()       != "" ? $('.note-editable').text().trim()       : null,//简介
    ...
    $scope.getEmployee = function(id){
        ...
        if(id != undefined){
            $http.get('/personnel/employee-details?EmployeeId='+id).then(function (result) {
                ...
                $http({method:'get',url:'/personnel/get-employee-position?branchId='+$scope.EmployeeData.organization_id+'&companyId='+$scope.modifyCompanyId}).then(function (data) {
                    $scope.getEmployeePositionData = data.data.attributes;
                },function (error) {console.log(error);Message.error("系统开了会小差,请刷新重试。。。")})
            })
        }else{
    ...
    $scope.EmployeeDataOrganization  = function (id) {
        ...
        if(id != ids){
            ...
            $http({method:'get',url:'/personnel/get-employee-position?branchId='+id+'&companyId='+$scope.modifyCompanyId}).then(function (data) {
                $scope.getEmployeePositionData = data.data.attributes;
            },function (error) {console.log(error);Message.error("系统开了会小差,请刷新重试。。。")})
            $scope.EmployeeData.position = ''
        }
    ...
    $scope.update = function(){
        $scope.employeeData = function(){
            return {
                ...
                identityCard            : $scope.EmployeeData.identityCard,     //身份证
                birth_time              : $('#update_birth').val(),        //出生日期
                ...
                work_url                :$scope.EmployeeData.work_url,//作品地址
                employeeIntro           :$('.note-editable').eq(1).html(),//简介
    ...
    $scope.employeeDetails = function (id,type,name) {
        ...
        if(display == 'none'){
            ...
            $(".recordCard").css("display","none");
            $(".presentaShow1").css("display","none") ;
            $(".userBasic1").css("display","block");
            $('.userBasic ').addClass('active');
            $('.userMessage').removeClass('active');
            $('.presentaShow').removeClass('active') ;
            $('.messageRecord').removeClass('active');
    ...
    $scope.employeeMemberInformation = function (id,name) {
        ...
        if (name == '私教部'){
            $http({method:'get',url:'/personnel/personality-details?employeeId='+id+'&type=1'}).then(function (data) {
                $scope.allUrl = data.data;
                $('.carousel').carousel();
            })
            $.loading.show();
    ...
    $scope.getRecordInfo = function(){
        //console.log($scope.employeeDetailsId);
        //console.log($scope.employeeMemberInfoId);
        $.loading.show();
        $http.get('/personnel/turn-member-info?employeeId=' + $scope.employeeDetailsId).then(function(result){
            //console.log(result);
            if(result.data.data.length != 0){
    ...
    $scope.presentaShow = function ()
    {
        $(".presentaShow").addClass('active');
        $(".userMessage").removeClass('active');
    ...
    $scope.messageRecord = function () {
        $(".userMessage").removeClass('active');
        $('.userBasic').removeClass('active');
        $('.messageRecord').addClass('active');
        $(".presentaShow").removeClass('active') ;
        $('.userMessage1').hide();
        $('.presentaShow1').hide();
        $('.userBasic1').hide();
    ...
    $scope.appointment = function(){
        ...
        $http({
            ...
            if(orderData.seatId != ''){
                ...
                //离职员工不能约课
                else if(success.data.status == 'leaveEmployee'){
