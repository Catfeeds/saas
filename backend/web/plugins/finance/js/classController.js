...
appModule.controller('classCtrl',function ($scope,$http,$timeout){
    ...
    $scope.searchList = function(){
        ...
        return{
            ...
            isImport  : $scope.isImport        != undefined  ? $scope.isImport        : null
        }
    };
    ...
    $scope.searchOpClick = function (type){
        $.loading.show();
        $scope.isImport = type;
        $scope.initPath();
        if($scope.isImport){
            $timeout($.loading.hide(),1000);
            // $.loading.hide();
            // console.log('asdadasd');
            location.href =  $scope.pageUrl;
            return true;
        }
        $scope.searchOp();
    };
