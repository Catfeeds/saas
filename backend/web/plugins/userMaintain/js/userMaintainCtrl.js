...
//    获取上过私教课的会员列表
    $scope.getMaintainUserList = function () {
        ...
        $.loading.show();
        var _url;
        if($scope.thisPageNum === undefined || $scope.thisPageNum === null || $scope.thisPageNum === '') {
            _url = '/user-maintain/fitness-member-list';
        }else {
            _url = '/user-maintain/fitness-member-list?per-page=8&page=' + $scope.thisPageNum;
        }
        $http.get(_url).then(function(results){
            // console.log(results);
    ...
    $scope.replacementPages = function (urlPages) {
        ...
        $http.get(urlPages).then(function(results){
            ...
            if(results.data != undefined && results.data != ""){
                ...
                $scope.navList = results.data.pages;
                $scope.thisPageNum = results.data.now;
            }else{
                $scope.maintainVenueFlag = true;
                $scope.maintainVenueMemberList = results.data.data;
                $scope.navList = results.data.pages;
...
//   新增饮食计划
    ...
    $scope.maintainEditBtn = function(){
        $('#maintainEdit').modal('show');
    };
    $('#maintainEdit').on('shown.bs.modal', function () {
        $scope.getTemplate();
    });
    //   获取模板列表
...
// 点击查看详情
    $scope.infoList=function(id){
        ...
        $http.get('/user-maintain/member-fitness-detail?memberId='+id).then(function(result){
            ...
            //初始化发送按钮不能点击
            if($scope.fitnessGoalDataName == '' ||  $scope.fitnessGoalDataName == null || $scope.fitnessGoalDataName == undefined){
                $scope.notSentGoalBtn = true;
            }else{
                $scope.notSentGoalBtn = false;
            }
            if($scope.foodPlanDataName == '' ||  $scope.foodPlanDataName == null || $scope.foodPlanDataName == undefined){
                $scope.notSentFoodBtn = true;
            }else{
                $scope.notSentFoodBtn = false;
            }
        })
        $scope.getGoals();
        $scope.getPlans();
    ...
    $scope.checkNumber = function(){
        var reg = /^[1-9]\d*$/;
        if ($scope.targetWeight != "") {
            if (!reg.test($scope.targetWeight)) {
                Message.warning('请输入正确的体重');
                $scope.targetWeight = '';
            }
        }
    }
        ...
        $scope.getMessages = function(){
            ...
            $http.get('/user-maintain/get-fitness-message?memberId='+$scope.memberIdData).then(function(res){
                ...
                if(res.data.length != 0){
                    $scope.showMessages = res.data;
                    $scope.detailNoData = false;
                }else{
                    $scope.showMessages = res.data;
                    $scope.detailNoData = true;
                }
            })
        };
        $scope.getMessages();
