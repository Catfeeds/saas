...
angular.module('App').controller('addManyClassServerCtrl',function($scope,$http,Upload){
    ...
    //验证多人阶梯价格
    $scope.peopleRule = function (classPersonRight,unitPrice,posPrice,lowNum) {
        if (classPersonRight.length == 0 || unitPrice.length == 0 || posPrice.length == 0 || lowNum.length == 0) {
            Message.warning('请将多人阶梯价格填写完整!');return false;
        }
        if ( classPersonRight.length != unitPrice.length || classPersonRight.length != posPrice.length || classPersonRight.length != lowNum.length) {
            Message.warning('请将多人阶梯价格填写完整!');return false;
        }
        return true;
    }
    ...
    $scope.rules = function () {
        ...
        if (isRepeat($scope.classId)) {
            Message.warning('选择的课程重复');
            return false;
        }
        if (!$scope.peopleRule($scope.classPersonRight,$scope.unitPrice,$scope.posPrice,$scope.lowNum)) { return false; }
        if (isRepeat($scope.classPersonRight)) {
            Message.warning('人数设置重复!');
            return false;
        }
