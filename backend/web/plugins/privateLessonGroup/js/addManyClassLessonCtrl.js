...
angular.module('App').controller('addManyClassLessonCtrl',function($scope,$http,Upload){
    ...
    $scope.getVenueOptions = function () {
        $http.get('/private-lesson-group/get-auth-venue').then(function (result) {
            if (result.data.status == 'success') {
                $scope.optionVenue = result.data.data;
    ...
    $scope.getPrice = function () {
        ...
        $div.each(function (i) {
            ...
            $limitPrice.each(function (i) {
                ...
                peopleArray[a] = $classPersonRight;
                a++;
            });
            $scope.classPrice.push($scope.limitDatas);
            $scope.peopleObj.push(peopleArray);
        })
    }
    //获取售卖场馆
    $scope.getVenueId = function () {
        ...
        console.log('$scope.venueId',$scope.venueId);
    }
                ...
                for (var i=0;i<arr1.length;i++) {
                    if (arr1[i].length>0) {
                        for (var j=0;j<arr1[i].length;j++) {
                            var arr = arr1[i][j];
                            if (arr['classPersonRight'] == '' || arr['classPersonRight'] == null || arr['classPersonRight'] == undefined) {
                                return 'classPersonRight';
                            }else if (arr['lowNum'] == '' || arr['lowNum'] == null || arr['lowNum'] == undefined) {
                                return 'lowNum';
                            }else if (arr['posPrice'] == '' || arr['posPrice'] == null || arr['posPrice'] == undefined) {
                                return 'posPrice';
                            }else if (arr['unitPrice'] == '' || arr['unitPrice'] == null || arr['unitPrice'] == undefined) {
                                return 'unitPrice';
                            }
                        }
                    }
    ...
    //计算总场馆售卖数量
    $scope.countSaleNum = function (arr){
        if ($scope.sellNum == -1) {
            //判断所有场馆售卖之和
            return true;
        }else {
    ...
    $scope.rules = function () {
        ...
        $scope.result = priceRule($scope.classPrice, $scope.intervalEnd);
        if ($scope.result == 'noAll') {
            Message.warning('多人阶梯价格填写不完整!');
            return false;
        }else if ($scope.result == 'classPersonRight') {
            Message.warning('请输入人数设置!');
            return false;
        }else if ($scope.result == 'lowNum') {
            Message.warning('请输入最低开课人数!');
            return false;
        }else if ($scope.result == 'posPrice') {
            Message.warning('请输入pos价格!');
            return false;
        }else if ($scope.result == 'unitPrice') {
            Message.warning('请输入优惠价格!');
            return false;
        }
        if (isRepeat($scope.intervalEnd)) {
            Message.warning('课程节数设置重复!');
            return false;
        }
        if ($scope.peopleObj) {
            for (var i=0;i<$scope.peopleObj.length;i++) {
                if (isRepeat($scope.peopleObj[i])) {
                    Message.warning('课程人数设置重复!');
                    return false;
                }
            }
        }
        if (!$scope.venueRule($scope.venueId,$scope.venueSaleNum)) { return false; }
        if (isRepeat($scope.venueId)) {
            Message.warning('售卖场馆重复!');
            return false;
        }
        if (!$scope.countSaleNum($scope.venueSaleNum)) {
            Message.warning('单馆售卖数量大于售卖总数量!');
            return false;
        }
        if (isRepeat($scope.giftId)) {
            Message.warning('赠品设置重复!');
            return false;
        }
