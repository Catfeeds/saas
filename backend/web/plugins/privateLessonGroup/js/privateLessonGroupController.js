...
angular.module('App').controller('privateLessonGroupCtrl',function($scope,$http,$timeout){
    ...
    $scope.attendClassData = function (url) {
        ...
        $http.get(url).then(function (result) {
            if(result.data.data.length === 0) {
                $scope.classDataInfo = true;
            }else {
                $scope.classDataInfo = false;
            }
    ...
    $scope.buttonSearchC = function () {
        ...
        $http({method:"get",url:'/private-lesson-group/charge-class?keywords='+$scope.keywords +'&venueId='+ $scope.chooseVenue+'&courseId='+$scope.chooseCourse}).then(function (result) {
            ...
            if(result.data.data == "" || result.data.data == undefined || result.data.data.length == undefined){
                $scope.chargeDataInfo = true;
            } else {
                $scope.chargeDataInfo = false;
            }
            $.loading.hide();
        },function (error) {
            Message.error('系统开了会小差,请刷新重试。。。');
    ...
    //获取购买私课区间
    $scope.getPriceLimitData = function (chargeId,name,pic,type,original_price,surplus_sale_num) {
        //类型:1多课程，2单课程
        if (type == 1) {
        ...
        $scope.surplus_sale_num = surplus_sale_num;
    ...
    $scope.dataInfos = '';
    $scope.primePrice = '';
    $scope.clickSelector = function (l,type) {
        $scope.classInfo = l;
        $scope.classInfo.type = type;
        $scope.cardNumber = '';
        $scope.userInfo   = '';
        //console.log('$scope.classInfo',$scope.classInfo);
        /*peopleId = $peopleId;
        $scope.cardNumber = '';
        ...
        $http.get('/private-lesson-group/check-member-info?cardNumber=' + $scope.cardNumber).then(function (response) {
            if (response.data.status == 'success') {
                ...
                if ($scope.userInfo.status == '2') {
                    Message.warning('会员卡异常,不能购课!');return;
                } else if ($scope.userInfo.status == '3') {
                    Message.warning('会员卡冻结,不能购课!');return;
                } else if ($scope.userInfo.status == '5') {
                    Message.warning('会员卡挂起,不能购课!');return;
                } else if ($scope.userInfo.status == '6') {
                    Message.warning('会员卡过期,不能购课!');return;
                }
                if (parseInt($scope.userInfo.current_time) > parseInt($scope.userInfo.invalid_time)) {
                    Message.warning('会员卡过期,不能购课!');return;
                }
                var minus = parseInt($scope.userInfo.current_time) - parseInt($scope.userInfo.create_at);
                if (parseInt(minus) < parseInt(24*60*60)) {
                    //购卡一天之内购课使用pos价格
                    $scope.userInfo.posType = 1;
                }else {
                    $scope.userInfo.posType = 2;
                }
                //判断应付价格
                if ($scope.userInfo.posType == 1) {
                    //走pos价格
                    if ($scope.classInfo.type == 1) {
                        //多课程,服务
                        $scope.primePrice = parseInt($scope.classInfo.pos_price);
                    } else if ($scope.classInfo.type == 2) {
                        //单课程
                        $scope.primePrice = parseInt($scope.classInfo.total_class_num) * parseInt($scope.classInfo.pos_price);
                    }
                }else if ($scope.userInfo.posType == 2) {
                    //走优惠价格
                    if ($scope.classInfo.type == 1) {
                        //多课程,服务
                        $scope.primePrice = parseInt($scope.classInfo.unit_price);
                    } else if ($scope.classInfo.type == 2) {
                        //单课程
                        $scope.primePrice = parseInt($scope.classInfo.total_class_num) * parseInt($scope.classInfo.unit_price);
                    }
                }
    ...
    $scope.getDatas = function () {
        ...
        var id;
        if ($scope.classInfo.type == 1) {
            id =  $scope.classInfo.id;
        }else if ($scope.classInfo.type == 2) {
            id = $scope.classInfo.class_people_id;
        }
        return {
            peopleId            : id,                                          //人数区间id
            ...
            course_amount       : $scope.classInfo.total_class_num,             //课程数量
            memberId            : $scope.userInfo.member_id,                             //会员id
            numberId            : $scope.classInfo.numberId,             //私课编号id
            memberCardId        : $scope.userInfo.id,                       //会员卡id
            _csrf_backend       : $('#_csrf').val()
        }
    }
