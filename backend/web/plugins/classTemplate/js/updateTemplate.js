...
angular.module('App').controller('updateTemplateCtrl',function($scope,$http,$timeout){
    //获取传递修改的ID值
    ...
    //获取编辑的内容
    $scope.getUpdateOneData = function(){
        $.loading.show();
        $http.get('/class-template/get-one-detail?id=' + $scope.updateId).success(function(result){
            //console.log(result);
            $scope.fileName = result.title; //模板名称
            $scope.describe = result.describe; //模板描述

            $scope.stages = result.stages; //阶段
            $scope.actionIdArr = [];
            $scope.stages.map(function(item,i){
                //console.log(item);
                $scope.actionId = [];
                item.main.map(function(item,i){
                    $scope.actionIdArr.push(item.action_id)
                });
            });
            $timeout(function(){
                $scope.getUpdateData();
            ...
            $.loading.hide();
            if(result.cid == 0){
                $('#select2-chooseType-container').text('请选择').attr('title','请选择');
            }else{
                $scope.chooseType = result.cid;//模板分类
            }
        })

    };
    $scope.getUpdateOneData();
    //展示训练内容
    $scope.getUpdateData = function(){
        $scope.actionIdArr.map(function(item,i){
            //$('.chooseSelect').val(item);
            $scope.oneId = item;
            $('.chooseSelect').eq(i).val($scope.oneId).trigger("change");
        });
    ...
    $scope.updateSubmitBtn = function(){
        ...
        $http({
            url : '/class-template/template-edit?id=' + $scope.updateId,
            method : 'POST',
