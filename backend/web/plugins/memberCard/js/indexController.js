...
app.controller('indexController',function($scope,$http,$timeout,Upload){
    ...
    //初始化获取场馆信息
    $scope.getVenueListInfo = function(){
...
//（数量不限）点击单选框，输入框解除限制
    ...
    //验证数组数据是否为空
    $scope.testArray = function(arr,text) {
        if (arr.length > 0) {
    ...
    $scope.sellingVenueEditComplete = function(){
        ...
        if (!$scope.testArray($scope.venueIds,'售卖场馆不能为空')) {
            return;
        }
        if (!$scope.testArray($scope.sheets,'售卖张数不能为空')) {
            return;
        }
        if (!$scope.testArray($scope.saleStart,'售卖日期不能为空')) {
            return;
        }
        if (!$scope.testArray($scope.saleEnd,'售卖日期不能为空')) {
            return;
        }
    ...
    $scope.leaveEditComplete = function(){
        ...
        if ($scope.isRepeat($scope.typeArr)) {
    ...
    //请假修改模态框弹出事件
    $('#leaveEditModal').on('shown.bs.modal', function (e) {
        //console.log('111')
        //console.log($scope.leaveDaysTotalEdit,$scope.leaveTimesTotalEdit)
        //如果请假总天数和每次请假最低天数都为空，下面的请假总次数和每次请假天数都可以点击
        if($scope.leaveDaysTotalEdit == '' && $scope.leaveTimesTotalEdit == ''){
            $scope.leaveNumsFlag = false;
            var $leaveLists = $('.leaveNumBoxLists').find('input');
            $leaveLists.each(function(){
                var $value = $(this).val();
            ...
            $('#addLeaveEdit').attr('disabled',false);
        }
    });
