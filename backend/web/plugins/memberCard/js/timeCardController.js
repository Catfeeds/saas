...
//获取合同内容
    ...
    $scope.addLeaveHtml = function () {
        ...
        $scope.htmlNum = $scope.htmlNum + 1;
        $http.get('/rechargeable-card-ctrl/add-venue?attr='+$scope.htmlAttr + '&num=' + $scope.htmlNum).then(function (result) {
...
//定义售卖场馆验证方法（attr数据数组，text提示内容）
    ...
    $scope.oneRules = function (data) {
        ...
        if(!$scope.commonRule(data.activeTime,'激活期限不能为空')){ return false; }
        ...
        if($('#areaMinPrice1').val() < 0 || $('#areaMinPrice2').val() < 0){
            Message.warning('请输入有效的一口价');
            return;
        }
        if($('#areaMinPrice1s').val() == '' && $('#areaMinPrice2s').val() == '' &&($('#areaMinPrice1').val() == ''||$('#areaMinPrice2').val() == '' )){
        ...
        if($scope.areaMinPrice != '' && $scope.areaMaxPrice != ''){
            if(parseInt($scope.areaMaxPrice) < parseInt($scope.areaMinPrice)){
...
//第二步-第二步验证
    ...
    // };
    //第二步数组验证
    $scope.testArray = function (arr1, arr2) {
        if (arr1.length > 0 ) {
    ...
    $scope.testArrayTwo = function (arr) {
        if (arr.length) {
    ...
    $scope.testTypeArr = function (arr) {
        var obj = {};
        for (var i in arr) {
            if (obj[arr[i]]) {
                return true;
    ...
    $rootScope.onStepChange = function (newIndex) {
        if(newIndex == 1){
           ...
           if(!$scope.assignParams($scope.OneParams,"one")){return false;}
            ...
            if (!$scope.testArray($scope.classId,$scope.pitchNum)) {
                Message.warning('请填写团课课程每日节数');
                return false;
            }
            //hs课程
           ...
           if(!$scope.assignParams($scope.TwoParams,'two')){return false;}
            ...
            if($scope.leaveDaysTotal != undefined && $scope.leaveDaysTotal != 0 && ($scope.leaveTimesTotal == undefined || $scope.leaveTimesTotal == 0)){
                ...
                return false;
            }else if ($scope.leaveNumsFlag) {
                //请假总天数,每次最低天数
                if ($scope.leaveDaysTotal) {
                    if (!$scope.leaveTimesTotal) {
                        Message.warning('每次最低请假天数不能为空');
                        return false;
                    }
                } else {
                    if ($scope.leaveTimesTotal) {
                        Message.warning('请假总天数');
                        return false;
                    }
                }
            }
            if (!$scope.testArrayTwo($scope.leaveDaysType)) {
                Message.warning('请假次数或每次请假天数不能为空');
                return false;
            }
            if ($scope.testTypeArr($scope.typeArr)) {
                Message.warning('每次请假天数不能重复');
                return false;
            }
