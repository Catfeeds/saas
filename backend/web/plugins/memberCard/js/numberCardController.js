...
//（数量不限）点击单选框，输入框解除限制
                ...
                if($moreSelect){
                    ...
                    $reslist.forEach(function(item,index,arr){
                        ...
                        if((index + 1) == $scope.everyLen){
                            $scope.leagueClass.push($scope.everClassArr);
                        }
                    })
    ...
    $scope.oneRules = function (data) {
        ...
        if(!$scope.commonRule(data.activationTime,'激活期限不能为空！')){ return false; }
        ...
        if(!$scope.commonRule(data.currencyTimes,'请填写通店次数！')){ return false; }
