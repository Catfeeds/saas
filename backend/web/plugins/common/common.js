        ...
        link: function (scope, elem, attrs) {
            $(elem).click(function() {
                if(attrs.id == 'addSellVenue'){
                    ...
                    return false;
                }else if(attrs.id == 'setDiscount') {
                    $(this).parents('.discountSettingAllBox').children().eq(0).append(scope.setDiscountHtml);
                    $compile(elem.parents('.discountSettingAllBox').find('.discountSettingBox').children(':last').contents())(scope);
                }else if(attrs.id == 'addUpgradeClass') {
                    $(this).parent().prev().eq(0).append(scope.addUpgradeClassHtml);
                    $compile($('.addUpgradePayBox').children(':last').contents())(scope);
                }else if(attrs.id == 'addDrillBtn'){
                    $(this).parents('.btnDiv').prev('.drillDiv').append(scope.setRoleHtml);
                    //服务器过滤，解决动态添加下拉没数据问题
                    $compile(elem.parents('.btnDiv').prev('.drillDiv').children(':last').contents())(scope);
                }else if(attrs.id == 'addTemplateBtn'){
                    $(this).parents('.addTemplateBtnDiv').prev('.templateBig').append(scope.templateHtml);
                    //服务器过滤，解决动态添加下拉没数据问题
                    $compile(elem.parents('.addTemplateBtnDiv').prev('.templateBig').children(':last').contents())(scope);
                }else if(attrs.id == 'addCustomBtn'){
                    $(this).parents('.addCustomBtnDiv').prev('.templateBig').append(scope.customHtml);
                    //服务器过滤，解决动态添加下拉没数据问题
                    $compile(elem.parents('.addCustomBtnDiv').prev('.templateBig').children(':last').contents())(scope);
                }else if(attrs.id == 'addChoiceBtn'){
                    $(this).parents('.addChoiceDiv').prev('.choiceDiv').append(scope.choiceHtml);
                    //服务器过滤，解决动态添加下拉没数据问题
                    $compile(elem.parents('.addChoiceDiv').prev('.choiceDiv').children(':last').contents())(scope);
                }else if(attrs.id == 'addMainBtn'){
                    $(this).parents('.addMainDiv').prev('.modalMainDiv').append(scope.mainHtml);
                    //服务器过滤，解决动态添加下拉没数据问题
                    $compile(elem.parents('.addMainDiv').prev('.modalMainDiv').children(':last').contents())(scope);
                }
...
angular.module('FilterApp',[]).filter('cut', function () {
    ...
    return function (value,key,type) {
            ...
            if (type == 1) {
                ...
                if (value*1000 + key*60*1000 < $nowTime) {
                    return true;
                } else {
                    return false;
                }
            ...
            for(var i=0;i < len;i++){
                if(value.id == attr[i].seat_id && value.seat_type == 3){
                    ...
                    if(attr[i].member.member_type == '2' ) {
                        ...
                        return 'btn-info';
                    }else if(attr[i].member.member_type == undefined) {
                        return 'btn-primary';
                    }
                }
            }
