angular.module('App').controller('orderCtrl', function($scope,$http){
    ...
    $scope.searchOrderData = function () {
        return {
            ...
            appBuy              : $scope.appBuy != undefined && $scope.appBuy != '' ? $scope.appBuy : null   //手机端购买
        }
    };
    //调用初始化方法
    $scope.init();
    ...
    //修改售卖人
    $scope.updateSellInfo = function(id,memberName,venueId,note,price,orderTime,employeeId,consumptionType){
    ...
    // 修改订单信息接口
    $scope.updatesell = function (id) {
        $scope.params = $scope.getUpdateParam();
        $http({
            url        : '/order/update-order-sell-people',
            method     : 'POST',
            data       :  $.param($scope.params),                               //
            headers    : { 'Content-Type': 'application/x-www-form-urlencoded'}
        }).then(function (result) {
            if(result.data.status == "success"){
                ...
                $('.modal-backdrop').hide();     //关闭模态框遮罩层
                Message.success("修改成功");
            }else if(result.data.status == "onlyone"){
                $scope.getOrderAllInfo();
                $('.modal').hide();              //关闭模态框
                $('.modal-backdrop').hide();     //关闭模态框遮罩层
                Message.warning("对不起，您没权限修改");
            }else{
                Message.warning('订单修改失败');
            }
        });
