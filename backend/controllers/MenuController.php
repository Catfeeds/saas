...
class MenuController extends \backend\controllers\BaseController
{
...
//        \Yii::$app->cache->flush();
        ...
        if ($model->load($post, '') && $model->validate()) {
            $data = $model->saveTopModule();//type默认值为1
            if ($data === true) {
...
//        \Yii::$app->cache->flush();
    ...
    {
        $model = new Module();
        $data  = $model->getTopModule(1);//1为pc
