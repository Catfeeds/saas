...
class ApiAboutRecordController extends ActiveController
{
    ...
    public function actionSetAboutRecord()
    {
        ...
        if(isset($post['wxType']) && $post['wxType'] == 'miniprogram'){
            //小程序解决同一账户不同帐号,潜在会员不能约课问题
            $mobile = (isset($post['mobile']) && !empty($post['mobile'])) ? $post['mobile'] : NULL;
            if(!$mobile){
                return ['code'=>0,'status'=>'error','message'=>'系统错误','data'=>[]];
            }
            $memberInfo = Member::find()->where(['mobile' => $mobile, 'company_id' => 1])->select('member_type')->asArray()->all();
        }else{
            $memberInfo = NULL;
        }
        ...
        if($about->load($post,'')&& $about->validate()){
            if($type == 'charge'){
                ...
                if(isset($post['wxType']) && $post['wxType'] == 'miniprogram'){
                    $save = $about->saveOrderTeamClass($memberInfo);
                }else{
