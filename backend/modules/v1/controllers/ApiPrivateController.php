...
class ApiPrivateController extends ActiveController
{
     ...
     *   }
     */
    public function actionGet($memberId,$requestType = '',$course = '')
    {
...
//        $data        = $group->search($params);
        if($requestType == 'ios')
        {
            $data = $group->getMemberOrder($memberId,$course);
            $model = new LoginForm();
            $model->member['id'] = $memberId;
     ...
     *   }
     */
    public function actionGetDetail($id,$memberId = 0,$requestType = '')
    {
        $group       = new ChargeClass();
        if($requestType == 'ios')
        {
