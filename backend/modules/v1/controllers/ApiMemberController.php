...
class ApiMemberController extends ActiveController
{
     ...
     * ｝
     */
    public function actionGetMemberClass($memberId, $requestType = '', $page = "")
    {
        $member = new Member();
        $arr = 'limit';
        if ($requestType == 'ios') {
            $data = $member->getMemberClassData($memberId, $arr, $page);
            if (!empty($data)) {
            ...
            return $member->getMemberClassData($memberId, $arr, "noPage");
        }
    }
     ...
     * ｝
     */
    public function actionGetWholeMemberClass($memberId, $requestType = '', $page = "")
    {
        $member = new Member();
        $arr = 'all';
        if ($requestType == 'ios') {
            $data = $member->getMemberClassData($memberId, $arr, $page, "wholeMemberClass");
            if (!empty($data)) {
                return ['code' => 1, 'status' => 'success', 'message' => '请求成功', 'data' => $data, "endPage" => $member->endPage];
            } else {
            ...
            return $member->getMemberClassData($memberId, $arr, $page = "noPage", "wholeMemberClass");
        }
    }
     ...
     * ｝
     */
    public function actionGetClassNotClass($memberId, $requestType = '', $page = "")
    {
        $member = new Member();
        if ($requestType == 'ios') {
            $data = $member->getMemberNotClassData($memberId, $page);
            if (!empty($data)) {
                return ['code' => 1, 'status' => 'success', 'message' => '请求成功', 'data' => $data, "endPage" => $member->endPage];
            } else {
            ...
            return $member->getMemberNotClassData($memberId, $page = "noPage");
        }
    }
     ...
     * ｝
     */
    public function actionGetClassCancelClass($memberId, $requestType = '', $page = "")
    {
        $member = new Member();
        if ($requestType == 'ios') {
            $data = $member->getMemberCancelClassData($memberId, $page);
            if (!empty($data)) {
                return ['code' => 1, 'status' => 'success', 'message' => '请求成功', 'data' => $data, "endPage" => $member->endPage];
            } else {
            ...
            return $member->getMemberCancelClassData($memberId, $page);
        }
    }
...
//                    return $result;
//                }
                $result = ['code' => 1, 'status' => 'success', 'message' => '登录成功', 'data' => $results];
                return $result;
            }
            $return = Func::setReturnMessageArr($model->errors, '登录失败');
            $result = ['code' => 0, 'status' => 'error', 'message' => $return, 'data' => $model->errors];
     ...
     * },
     */
    public function actionGetMemberOne($memberId)
    {
        $model = new LoginForm();
        $mOne = $model->getMemberOne($memberId);
        if (!empty($mOne)) {
...
//                ,"extraData"=>$extraData,"belMessage"=>$model->memberBelMessage 老流程返回数据
     ...
     * }
     */
    public function actionGetMemberInfo($memberCardId, $memberId)
    {
        $model = new MemberCard();
        $data = $model->getMemberCardInfo($memberCardId, $memberId);
        if (!empty($data)) {
     ...
     * <span><strong>邮    箱：</strong></span>huangpengju@itsprts.club
     * <span><strong>创建时间：</strong></span>2017/6/27
     * @apiSampleRequest  http://qa.aixingfu.net/v1/api-member/get-member-card-all
     * @apiSuccess (返回值) {string} data
     * @apiSuccessExample {json}返回值详情(成功时)
     * {
     *  "code": 1,              //成功标识
     ...
     * }
     */
    public function actionGetMemberCardAll($memberId, $venueId = null)
    {
        $model = new MemberCard();
        $data = $model->getMemberCardAll($memberId, $venueId);
        if (!empty($data)) {
     ...
     * }
     */
    public function actionGetMemberCabinet($memberId)
    {
        $model = new MemberCard();
        // 会员身份判断
        $memberStatus = \common\models\base\Member::findOne($memberId);
        $this->memberStatus = (empty($memberStatus) || !isset($memberStatus->member_type)) ? 2 : $memberStatus->member_type;
        $data = $model->getMemberCabinet($memberId);
        if (!empty($data)) {
            return ['code' => 1, 'message' => '请求成功', 'data' => $data, "memberStatus" => $this->memberStatus];
        } else {
            return ['code' => 1, 'message' => '您未租用柜子', 'data' => '该会员未租用柜子', "memberStatus" => $this->memberStatus];
        }
    }
...
//            }
    ...
    public function actionIsUpdate($appType = "", $venue = "", $versionCode = null)
    {
        ...
        $url = 'https://itunes.apple.com/us/app/id1290824220';   //这儿填页面地址
        $info = file_get_contents($url);
        preg_match('/<span itemprop="softwareVersion">(.*?)<\/span>/i', $info, $m);
        $version = isset($m[1]) && !empty($m[1]) ? $m[1] : null;
        if ($version == $versionCode) {
            return ['code' => 1, 'message' => "请求正常", "update" => 1, "hasVersion" => false];
        }
        //查找配置
        $config = new Config();
        $update = $config->gainAppConfig($appType, $venue);
        return ['code' => 1, 'message' => "请求正常", "update" => $update, "hasVersion" => true, "lineVersion" => $version];
    }
...
//--------------------------------------------------------会员更柜管理--------------------------------------------------
    ...
    {
        $model = new Cabinet();
        $data = $model->getAllCabinetData($venueId);
     ...
     * }
     */
    public function actionMemberLeaveRecord($memberId = "")
    {
        $model = new Member();
        $endResult = $model->memberLeaveRecord($memberId);
        if (empty($endResult)) {
