...
class ModifyPasswordForm extends Model
{
    ...
    public function rules()
    {
        return [
            ...
            ['venueMemberId', 'required', 'message' => '会员不能为空！'],
    ...
    {
        if(empty($this->venueMemberId)){
            $member = MemberAccount::findOne(['mobile' => $this->mobile]);
        }else{
            $mm = Member::findOne($this->venueMemberId);
            $member = MemberAccount::findOne(['id' => $mm->member_account_id]);
        }
    ...
    public function getMemberOne()
    {
        if (!isset($this->member) && empty($this->member)) {
            ...
            if(empty($this->venueMemberId)){
                ...
                $this->member = $member->getMemberOneData($this->venueMemberId);
                if(!empty($this->member)){
                    $this->member = MemberAccount::find()->where(['id'=>$this->member['member_account_id']])->asArray()->one();
                }
            }
        }
        return $this->member;
