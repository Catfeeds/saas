...
class AboutRecordForm extends Model
{
    ...
    public function saveAbout()
    {
        ...
        if(isset($this->type) && ($this->type == 'PC')){
            ...
            $reAbout = AboutClass::find()
                ...
                ->andWhere(['<>','status',2])
                ->andWhere(['or',['between','start',$this->start,$this->end],['between','end',$this->start,$this->end]])
                ->asArray()->one();
            if(isset($reAbout)){
                return '此会员在该时间段已预约课程';
            }
            $reCoach = AboutClass::find()
            ...
            $about->about_type       = ($this->aboutType == 'mobile') ? 1 : 2;                                            //预约方式(1电话预约2自助预约）
        }else{
            ...
            $about->member_id        = $this->memberId;
            $about->is_print_receipt = 2;                                                                                 //是否打印小票（1有2没有）
            $about->about_type       = ($this->aboutType == 'mobile') ? 1 : 2;
        }
        if($about->save()){
    ...
    public function saveGroupAbout()
    {
        ...
        {
            $num = \backend\models\AboutClass::find()->alias('ac')->joinWith(['groupClass gc'])->where(['ac.member_id'=>$this->memberId,'gc.venue_id'=>$this->potVenueId,'ac.type'=>2])->andWhere(['<>','ac.status',2])->count();
            if($num >=1){
...
//                if($this->aboutType == 'app' || $this->aboutType == 'pc'){
...
//                    }
            ...
            }elseif (($memberCard['create_at'] < strtotime('2017-6-18'))&&($this->aboutType=="app") || $this->aboutType == 'pc'){       // app老会员预约课程
                // 开课前多少分钟不能预约
                $aboutLimit = $this->aboutLimit();
                if($aboutLimit == null || $aboutLimit == 1) {
                    $beforeClassLimit = $this->beforeClassLimit();
    ...
    /**
     * 云运动-小程序-团课预约
     * @author 杨慧磊 <lihuien@itsports.club>
     * @create 2017/4/24
     * @param $memberInfo
     * @return array|\yii\db\ActiveRecord[]
     */

    public function saveOrderTeamClass($memberInfo)
    {
        if(!$memberInfo){
            return false;
        }
        ...
        $this->getGroupClassId();                                                    //处理团课预约时间
        $memberType = array_column($memberInfo, 'member_type');
        $this->getMemberId();                                                        //处理会员单条数据（获取会员卡信息）
        if(!in_array(1, $memberType))
        {
        ...
        $group = \backend\models\GroupClass::find()
            ...
            ->asArray()
            ->one();
        if(empty($this->is_employee) && in_array(1, $memberType)) {                 //不是员工，或者不是潜在会员的 走if里面
//            \Yii::trace($memberCard,'$memberCard2');
            if(time() > $memberCard['invalid_time']) return 'expired';//会员卡过期
            if($memberCard['create_at'] > strtotime('2017-6-18')){
                // 调取会员卡 和 会员卡id
...
//                    }
        ...
        $about->status           = 1;
        $about->member_card_id   = $this->memberCardId;
        $about->member_id        = \common\models\MemberCard::findOne(['id' => $this->memberCardId])->member_id;
        $about->is_print_receipt = 2;                                                               //是否打印小票（1有2没有）
        $about->about_type       = ($this->aboutType == 'mobile') ? 1 : 2;                         //预约方式(1电话预约2自助预约）
        $about->employee_id      = $this->employeeMemberId;                                        //员工id
        if($about->save()){
