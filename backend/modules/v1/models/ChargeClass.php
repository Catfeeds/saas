...
//            )
     ...
     * @return array|\yii\db\ActiveRecord[]
     */
    public function getMemberOrder($memberId, $course = '')
    {
        // 根据场馆需求 暂时隐藏数据
        $data = MemberCourseOrder::find()
            ->alias('order')
            ->joinWith(['memberCourseOrderDetails details'])
            ->where(['order.member_id' => $memberId, 'order.class_number_id'=>NULL])//过滤掉小团体课
...
//        }
    ...
    {
        return \common\models\base\ChargeClassPrice::find()->where(['charge_class_id'=>$this->id])->select('intervalStart,intervalEnd,unitPrice,posPrice,app_discount')->asArray()->all();
    }
    /**
     *  云运动 - api接口 - 查询课程详情
     * @author huangpengju<huangpengju@sports.club>
