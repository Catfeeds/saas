...
class MemberCard extends \common\models\base\MemberCard
{
   ...
   public function getMemberCardNum($memberId,$status=[1])
   {
     ...
     * @inheritdoc
     */
   public function getAllMemberCard($memberId,$status=[1]){
       $member = MemberCard::find()
           ->where(['member_id'=>$memberId])
           ->andWhere(["in",'status',$status])
           ->andWhere(['>','invalid_time',time()])
...
//      $data = $this->serArrayClassId($card);        //获取多态id(课程id等等)
     ...
     */
    public function getMemberCardInfo($memberCardId,$memberId)
    {
        $memberCard = \backend\models\MemberCard::find()
           ...
           if($memberCard['status'] !=4)
           {
               ...
               $memberCard['lastTime']    = (int)ceil(($memberCard['invalidTime'] - time())/86400);        //剩余天数
     ...
     */
    public function getMemberCardAll($memberId,$venueId)
    {
        //类实例化  初始化赋值   获取场馆冻结方式
        $leagueModel = new AbsentRecord();
        $missRecords = new MissRecords();
        $freeze      = $leagueModel->gainFreezeWay($venueId);   // 场馆冻结方式
        $missRecords->venueId = $venueId;
     ...
     * @return string
     */
    public function gainSql($memberId){
        $sql =  \backend\models\MemberCard::find()
            ->alias("memberCard")->where(["and",['>','memberCard.invalid_time',time()],['memberCard.member_id'=>$memberId]])
    ...
    public function  delSql($type,$sql,$customDay){
        if($type==1){
             ...
             $data = $sql->select("memberCard.id,memberCard.id as memberCardId,memberCard.card_number as cardNumber,,memberCard.pic,
            ...
            memberCard.absentTimes,memberCard.recent_freeze_reason,memberCard.recent_freeze_reason,missAboutSet.freeze_way,missAboutSet.miss_about_times,missAboutSet.punish_money,(missAboutSet.miss_about_times-memberCard.absentTimes) as surplusTimes,
        ...
        if($type==2){
            ...
            $lastMonthTime = strtotime(date('Y-m-t'));               // 获取当前月最后一天
            $data = $sql->select("memberCard.id,memberCard.id as memberCardId,memberCard.card_number as cardNumber,
    ...
    {   
        ...
        $data     = $cabinet->getMemberCabinetAll($memberId);
        return $data;
    }
    /**
     * @云运动 - API - 获取会员柜子详细信息
    ...
    public function gainAbsentRecord($identify,$identifyId,$courseType){
        ...
        if($identify=="card"&&($courseType!=3)){           // 卡 courseType 1代表私课 2代表团课 旷课记录
            ...
            $data = $data->andWhere(["and",["aboutClass.member_id"=>$identifyId],["aboutClass.type"=>$courseType]]);
        }else{
            $data = $data->andWhere(["aboutClass.member_id"=>$identifyId]);
        }
    ...
    public function dealPrivateCourseName($data){
      ...
      foreach ($data as $keys=>$value){
          if(empty($data[$keys]["courseName"])){
              $data[$keys]["courseName"] = $data[$keys]["course_name"];
          }
      }
      return $data;
    }
    ...
    public function gainPublicSql(){
        $data = AboutClass::find()
            ...
            ->select("
                      ...
                      memberCourseOrderDetails.course_name,
