...
class Member extends \common\models\base\Member
{
     ...
     * @return array
     */
    public function getMemberClassData($memberId,$arr,$page,$type = "")
    {
       $this->memberId  = $memberId;                                     // 会员id
       $this->status    = '';                                            // 约课课程状态
       $this->page      = $page;                                         // 分页
       $this->requestSource ="recentCourse";                          // 会员最近课程
       $classData        = $this->getDataMemberClassById($memberId,$arr);  //会员预约团课课程
     ...
     * @return array
     */
    public function getMemberNotClassData($memberId,$page)
    {
        $this->memberId = $memberId;
        $this->status   = 1;
        $this->requestSource = "unusedCourse";
        $this->page      = $page;
        $classData       = $this->getDataMemberClassById($memberId,'last');                 //会员预约团课课程
     ...
     * @return array
     */
    public function getMemberCancelClassData($memberId,$page)
    {
        $this->status   = 2;            // 取消的课程
        $this->type     = 'cancel';
        $this->memberId = $memberId;    // 会员id
        ...
        $classData       = $this->getDataMemberClassById($memberId,'');          //会员预约团课课程
    ...
    {
        $member =  MemberCard::find()->select(['id'])->where(['member_id'=>$memberId])->asArray()->one();
        if($member){
            return $member['id'];
        }else{
            return NULL;
     ...
     * @return array
     */
    public function getAboutClassId($memberId,$time)
    {
        $id                   = $this->getMemberId($memberId);         //获取会员卡id
     ...
     * @return array
     */
    public function getAboutChargeId($memberId,$time)
    {
        $id                   = $this->getMemberId($memberId);                //获取会员卡id
        $this->member_card_id = $id;
     ...
     * @return array
     */
    public function getClassGroupId($memberId)
    {
        $id      = $this->getMemberId($memberId);
     ...
     * @return array
     */
    public function getClassChargeId($memberId)
    {
        $id      = $this->getMemberId($memberId);
     ...
     * @return array
     */
    public function getDataMemberClassById($memberId,$arr)
    {
        if($arr == 'limit')
        {
            $time  = strtotime('-3 day');
            $classId = $this->getAboutClassId($memberId,$time);                                //获取预约团课的课程id（数组）
        }else if($arr == 'last'){
            $classId = $this->getAboutClassId($memberId,'');                                   //获取预约团课的课程id（数组）
        }else{
            $classId = $this->getAboutClassId($memberId,'');                                   //获取预约团课的课程id（数组）
        }
        //  执行团课部分数据逻辑代码
    ...
    public function getFieldsByOne($v,$type)
    {
        switch ($type){
            ...
            case 'aboutData':
                ...
                return $about;
            case 'aboutCharge':
                $about       = AboutClass::getAboutClassOneBy($v['id'],$this->memberId,1);
                if($about)return $about[0];
                return $about;
            case 'score':
...
//        }
    ...
    {
       return AboutClass::find()->where(['class_id'=>$id])
...
// ------------------------------------赠送金额 数据处理 -------------------
     ...
     */
    public function memberLeaveRecord($memberId){
        ...
        $data = \backend\models\LeaveRecord::find()
            ->alias("leaveRecord")
            ->where(["leaveRecord.leave_employee_id"=>$memberId])
