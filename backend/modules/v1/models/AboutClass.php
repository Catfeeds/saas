...
//            $data['start']            = date('m月d日 H:i',$data['start']);
     ...
     * @return array|\yii\db\ActiveRecord[]
     */
    public function getNewAboutOne($id,$start,$coaId,$mcId,$mId,$venueId)
    {
        $data = [];
        $arr  = [];
        ...
        if(isset($data) && $data){
            ...
            $arr['venue_name']          = $order['venueName'];                            //场馆名称
            $arr['venue_address']       = empty($order['venueAddress'])?'场馆地址:暂无数据':$order['venueAddress']; //场馆地址
            $arr['chargeNum']        = count($this->getChargeNum($id,1,$data['member_card_id'],$id));  //正在上第几节课
            $model = new ChargeClass();
            $score                   = $model->getFieldsByOne($v = [],'score');
...
//                $query->joinWith(['organizations or']);
            ...
            ->select('
                ...
                course.venue_id as courseVenueId,or.id as venueId,or.name as venueName,or.address as venueAddress,orderList.venue_id,
              ')
            ->asArray()
            ->one();
...
//        }
     ...
     * @return array|\yii\db\ActiveRecord[]
     */
    public function getChargeNum($classId,$type,$memberId,$id)
    {
        $num = AboutClass::find()->where(['class_id'=>$classId])
            ->andWhere(['type'=>$type])
            ->andWhere(['member_card_id'=>$memberId])
