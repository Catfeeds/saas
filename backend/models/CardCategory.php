...
//        }
    ...
    {
        if ($text == 'ban') {
            return self::editStatusBen($id);    //冻结
        } else {
            return self::editStatusTime($id);   //过期
        }
    }
    ...
    public static function getTemplate($attr)
    {
        ...
        } else if ($attr == 'addCabinetMonth') {
            $html = '/new-cabinet/_add_month_cabinet';
        } else if ($attr == 'modifyPlugins') {
            $html = '/new-cabinet/new_add_cabinet';
        } else if ($attr == 'svenueHtml123') {
        ...
        } else if ($attr == 'payWaysHtml') {
            $html = 'add-children/payHtml';
        } else if ($attr == 'addNewDiscountSetting') {
            $html = '/private-lesson/add-children/addNewDiscountSetting';
        } else if ($attr == 'addUpgradeClass') {
            $html = '/member/add-children/addUpgradeClass';
        } else if ($attr == 'addExample') {
            $html = '/action-library/add-example/addExample';
        } else if ($attr == 'addDrill') {
            $html = '/class-template/add-children/drill';
        } else if ($attr == 'addTemplate') {
            $html = '/class-template/add-children/template';
        } else if ($attr == 'addSelfTemplate') {
            $html = '/class-template/add-children/custom';
        } else if ($attr == 'addChoice') {
            $html = '/private-class-set/add-children/choice';
        } else if ($attr == 'addMain') {
            $html = '/private-class-set/add-children/main';
        } else {
            $html = 'add-children/venue';
        }
...
//            ->andWhere(['goods.goods_attribute' => 2])
     ...
     * @return boolean
     */
    public function getLeaveCardData()
    {
        $query = CardCategory::find()->alias('cc')->select("cc.id,cc.card_name")->asArray();
        $query = $this->getSearchWhereData($query);
        $query = $query->all();
     ...
     * @return  mixed
     */
    public function getSearchWhereData($query)
    {
        $query->andFilterWhere(['cc.venue_id' => Config::accessVenues()]);
