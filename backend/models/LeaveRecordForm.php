...
class LeaveRecordForm extends Model{
    ...
    public function leaveRecord($requestSource = ""){
        ...
        if($this->leaveType == 1){
            ...
            $registrationData = LeaveRecord::find()
                ->where(['and',['leave_employee_id'=>$this->leavePersonId],['member_card_id'=>$this->memberCardId]])
                ->andWhere(['and',['status'=>4],['>','leave_end_time',time()]])
                ->asArray()->one();
        ...
        }elseif($this->leaveType == 2){//特殊请假
            //查询请假记录表看该卡是否有未处理的请假记录（cloud_leave_record->type 1待处理,2已同意,3已拒绝'），有return
            $record = LeaveRecord::find()
                ->where(['member_card_id' => $this->memberCardId,'type' =>1])
                ->asArray()->all();
            if($record){
                return '对不起，您还有特殊请假未处理，暂时不能请假！';
            }
        ...
        if($this->leaveType == 2){
            $model->status          = 3;//特殊请假 已登记
        }else if($this->leaveType == 1 && ($this->leaveStartTime)<=time()){
            $model->status          = 1;//请假中 2已销假
        }else if($this->leaveType == 1 && ($this->leaveStartTime)>time()){
            $model->status          = 4;// 已登记
        }else if($this->leaveType == 3 && ($this->leaveStartTime)<=time()){
            $model->status          = 1;// 学生请假假期中
        }else if($this->leaveType == 3 && ($this->leaveStartTime)>time()){
            $model->status          = 4;// 学生请假已登记
        }
        ...
        if($this->leaveType == 2){
            $model->leave_property  = 1;//1特殊请假
        }else if($this->leaveType == 1){
            $model->leave_property  = 2;//2正常请假
        }else{
