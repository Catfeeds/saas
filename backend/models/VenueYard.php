...
Class VenueYard extends \common\models\base\VenueYard
{
    ...
    public function getVenueYardData($search)
    {
        ...
        $data = VenueYard::find()->alias("venueYard")
            ...
            ->joinWith(["organizationRoom or"], false)
            //   ->joinWith(["venueYardCardCategory yardCardCategory"],false)
            ->select("venueYard.*,organization.name,or.name as roomName")
...
//        }
    ...
    public function getVenueAllAboutRecord($venueId,$cardNum){
        ...
        $aboutData = \backend\models\AboutYard::find()
                    ...
                    ->select("aboutYard.id,aboutYard.is_print_receipt,aboutYard.yard_id,aboutYard.aboutDate,aboutYard.about_interval_section,venueYard.yard_name,organization.name as venueName")
                    ->asArray();

        $data  = Func::getDataProvider($aboutData,5);

        return $data;
    }
    public function cancelYardAbout($yardId,$cardId,$intervalSection,$memberAboutDate,$memberIdB,$memberIdT){
        if($memberIdB == $memberIdT)
        {
