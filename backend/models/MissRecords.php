...
Class MissRecords extends  AboutClass{
    ...
    public function MonthBeginFreeze($type){
        ...
        MemberCard::updateAll(["absentTimes"=>0,"status"=>1,"recent_freeze_reason"=>2,'last_freeze_time'=>time()],["id"=>$freezeMemberCardId]);
    }
    /**
     * 后台 - 新团课管理 -爽约（自动解冻（两种方式）） - 解冻后默认值的设置
     * @author  <houkaixin@itsports.club>
    ...
    public function checkAllFrozenCard($type){
        ...
        if($type==1){
            ...
            if(empty($this->isRequestMember)){   // 针对 所有卡的自动解冻
                ...
                $freezeMemberCardId = $freezeMemberCardId->andWhere(["member_id"=>$this->memberId])->select("id,absentTimes,last_freeze_time,({$freezeTime}+last_freeze_time) as endTime")->asArray()->all();
            }
   ...
   public function gainTheSql(){
    ...
    public function delSql($query){
       ...
       if($this->freezeWay==1){                                 // 当月冻结到月末
           ...
           $query = $query->select("memberCard.id,memberCard.member_id,memberCard.card_number,memberCard.invalid_time,memberCard.status,
                         ...
                         missAboutSet.freeze_way,missAboutSet.punish_money,ceil((({$lastMonthTime}-{$time})/(60*60*24))) as surplusDay,memberCard.create_at");
       }
       if($this->freezeWay==2){                               //自定义冻结天数
           ...
           $query =  $query->select("memberCard.id,memberCard.member_id,memberCard.card_number,memberCard.invalid_time,memberCard.status,
                         memberCard.absentTimes,memberCard.frozen_start_time,memberCard.frozen_end_time,memberCard.recent_freeze_reason,
                         MemberDetail.name as memberName,member.mobile,memberDetail.sex as memberSex,
                         missAboutSet.freeze_way,missAboutSet.punish_money,floor((memberCard.last_freeze_time+{$endTime}-{$time})/(24*60*60)) as surplusDay,memberCard.create_at");
       }
       if($this->freezeWay==3){
           $query =  $query->select("memberCard.id,memberCard.member_id,memberCard.card_number,memberCard.invalid_time,memberCard.status,
                         memberCard.absentTimes,memberCard.frozen_start_time,memberCard.frozen_end_time,memberCard.recent_freeze_reason,
    ...
    public function delMissSet($venueId){
        ...
        //查找当前场馆的所有会员
        $memberAll = Member::find()->where(['venue_id'=>$venueId])->column();
        //解冻当前场馆所有因爽约被冻结的卡
        MemberCard::updateAll(["status"=>1,"recent_freeze_reason"=>2,"last_freeze_time"=>time()],["and",["member_id"=>$memberAll],["status"=>3],["recent_freeze_reason"=>1]]);
        MemberCard::updateAll(['absentTimes'=>0,'last_freeze_time'=>time()],['member_id'=>$memberAll]);
        if(empty($data)){
            return true;
        }
        $data->delete();
