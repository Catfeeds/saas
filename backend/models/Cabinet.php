    ...
    public function getData($params, $requestType = "", $pageSize = 8)
    {
        ...
        $query = Cabinet::find()
            ...
            ->joinWith(["memCabinet" => function ($query) {
                ...
                $query->joinWith(['memberCard mc' => function ($query) {
                    $query->onCondition(['<>', 'mc.status', 2])->orderBy('mc.invalid_time desc')->onCondition(['or', ['<>', 'mc.usage_mode', 2], ['mc.usage_mode' => null]]);
                }]);
            }])
            ->select("
               cloud_cabinet.*,
...
////        ]);
     ...
     * @return array
     */
    public function getAllCabinetData($id)
    {
     ...
     * @return array
     */
    public function getSearch($query, $id)
    {
    ...
    private function setSearchWhere($data, $venueId)
    {
        if (isset($venueId) && $venueId != null && $venueId != '' && $venueId != 'undefined') {
            ...
            $query = $data->andFilterWhere(['cloud_cabinet_type.venue_id' => \backend\rbac\Config::accessVenues()]);
        }
    ...
    /**
     * @desc: 营运管理-更柜管理-获取单个柜子的详细信息
     * @author: 付钟超 <fuzhongchao@itsports.club>
     * @create: 2018/03/08
     */
    public function getCabinetDetails($cabinet_id)
    {
        $data = Cabinet::find()
            ->alias('cabinet')
            ->joinWith(["cabinetType cabinetType"], false)
            ->joinWith(['memberCabinet mcc'], false)
            ->joinWith(["memberCabinetRentHistory memberCabinetRentHistory"], false)
            //->joinWith(["admin"],false)
            ->joinWith(["memberCabinet mt" => function ($query) {
                $query->joinWith(["member member" => function ($query) {
                    $query->joinWith(["memberDetails mds"], false);
                    $query->joinWith(['memberCard mc'], false);
                    $query->joinWith(['employee employee'], false);
                }, "employee"]);
            }], false)
            ->select("
                mt.id as memberCabinetId,
                mt.member_id,
                mt.price,
                mt.start_rent,
                mt.end_rent,
                mt.back_rent,
                memberCabinetRentHistory.remark,
                cabinet.id as cabinet_id,
                cabinet.cabinet_type_id,
                cabinet.cabinet_number,
                cabinet.cabinet_model,
                cabinet.cabinet_type,
                cabinet.deposit,
                cabinet.give_month,
                cabinet.cabinet_month,
                cabinet.cabinet_money,
                cabinet.cabinet_dis,
                cabinet.monthRentPrice,
                cabinet.status,
                member.username,
                mds.pic,
                member.mobile,
                ...
                member.member_type,
                employee.name,
                cabinetType.type_name,
                mcc.end_rent,
                mc.status as mc_status,
                mds.note,
                    ")
            ->andWhere(['cabinet.id' => $cabinet_id])
            //->createCommand()->rawSql;
            ->asArray()->one();
        $data = $this->makeCombineData($data);
        return $data;
    }

    /**
     * @desc: 营运管理-更柜管理-组装单个柜子的到期日期
     * @author: 付钟超 <fuzhongchao@itsports.club>
     * @create: 2018/03/08
     * @param $data
     * @return mixed
     */
    public function makeCombineData($data)
    {
        if (!empty($data)) {
            if (!empty($data["end_rent"])) {
                if (!empty($data["back_rent"])) {
                    if ($data["back_rent"] >= time() && $data["end_rent"] > $data["back_rent"]) {
                        $surplusDay = ceil(((int)$data["back_rent"] - time()) / 86400);
                    } else {
                        $surplusDay = 0;
                    }
                } else if (time() > $data["end_rent"]) {
                    $surplusDay = ceil(((int)$data["end_rent"] - time()) / 86400);
                } else {
                    $surplusDay = ceil(((int)$data["end_rent"] - time()) / 86400);
                }
                $totalDay = ceil((int)$data["end_rent"] - $data["start_rent"]) / 86400;
            } else {
                $surplusDay = false;
                $totalDay = false;
            }
            $data["totalDay"] = $totalDay;
            $data["surplusDay"] = $surplusDay;
        }
        return $data;
    }
}