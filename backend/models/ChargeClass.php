...
class ChargeClass extends \common\models\ChargeClass
{
     ...
     * 私教管理 - 获取私教小团体 - 多表联查
     * */
    public function searchGroup($params,$venueId,$companyId)
    {
        $this->customLoadGroup($params,$venueId);
        $this->saleVenueId = $venueId;
        //修改过期课程
        \common\models\ChargeClass::updateAll(['status'=>3],['and',['group'=>2],['category'=>2],['<','sale_end_time',time()]]);
        $query = ChargeClass::find()
            ->alias('cc')
            ->joinWith(['coursePackageDetail dd' => function($query){
            ...
            ->joinWith(['chargeClassNumber ccn' => function($query){
                $query->orderBy('ccn.surplus_sale_num');
            }])
            ...
            ->where(['cc.category'=>2])
            ->select(
                "cc.*,course.name as courseName,organization.name as venueName,
         ccn.sale_num,ccn.surplus_sale_num,UNIX_TIMESTAMP() as now_time")
            ->groupBy('dd.charge_class_id')
            //->andWhere(['cc.company_id'=>$companyId])
            //->createCommand()->getRawSql();
            ->asArray();
        if (\Yii::$app->user->identity->level != 0) {
            $query->andFilterWhere(['cc.company_id'=>$companyId]);
            $query->joinWith(['classSaleVenueOne saleVenue' =>function($query){
                $query->where(['saleVenue.venue_id'=>$this->saleVenueId]);
            }],false);
        }
        $query        = $this->getSearchWhere($query);
        $dataProvider = Func::getDataProvider($query,8);
    ...
    /**
     * @desc: 业务后台 - 私教小团体 - 搜索数据处理
     * @author: 付钟超 <fuzhongchao@itsports.club>
     * @create: 2018/03/21
     * @param $params
     * @param $venueId
     */
    public function customLoadGroup($data,$venueId)
    {
        ...
        $this->venueId       = (isset($data[self::VENUE_ID]) && !empty($data[self::VENUE_ID])) ? $data[self::VENUE_ID] : null;
        $this->keywords      = (isset($data['keywords']) && !empty($data['keywords'])) ? $data['keywords'] : null;
        $this->courseId      = (isset($data['courseId']) && !empty($data['courseId'])) ? (int)$data['courseId'] : null;
    }
    /**
...
//        }
    ...
    public function getClassManyInfo($memberId,$venueId)
    {
        ...
        $data  = ChargeClass::find()
            ...
            ->select(
                "class.id,
                class.name as packName,
                class.pic,
                class.original_price,
                class.total_amount,
                class.total_pos_price,
                class.type,
                class.valid_time,
                csvs.id as saleVenue")
            ->asArray();
...
//                unset($v['chargeClassPriceAll']);
    ...
    public function chargePrice($chargeClassId)
    {
        $details = ChargeClassPrice::find()
            ...
            ->select('intervalStart,intervalEnd,unitPrice,posPrice,app_discount')
            ->asArray()
            ->all();
        return $details;
    }
     ...
     * @return array
     */
    public function getCourseHsData($venueId)
    {
        return  ChargeClass::find()
            ->alias('cc')
            ->where(['and',['cc.venue_id'=>$venueId],['course_type'=>2]])
            ->andWhere(['not like','cc.name','生日'])
            ->asArray()
            ->all();
    }
