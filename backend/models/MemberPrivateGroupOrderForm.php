...
class MemberPrivateGroupOrderForm extends \common\models\MemberCourseOrder
{
    ...
    public $type;                                       //1.服务 2.课程
    public $month_up_num;                               //课程每月上课数量
    public $valid_time;                                 //服务有效期
    /**
    ...
    public function rules()
    {
        return [
            [[
                'memberId','peopleId','coachId','saleType','payType','giftType','primePrice','memberCardId',
   ...
   public function getChargeClassInfo()
   {
       ...
       if ($this->type == 2) {
           $this->month_up_num = isset($data['month_up_num']) ? $data['month_up_num'] : null;
       }elseif ($this->type == 1) {
           $this->valid_time = isset($data['valid_time']) ? $data['valid_time'] : null;
       }
   }
   ...
   public function getMemberCardId()
   {
    ...
    public function saveMemberCourseOrder()
    {
        ...
        if ($this->type == 2) {
            if ((int)$this->courseAmount > (int)$this->month_up_num) {
                $month = ceil((int)$this->courseAmount/(int)$this->month_up_num);
            }else{
                $month = 1;
            }
            $model->deadline_time       = (int)strtotime($this->create_at) + (int)($month*30*24*60*60);       //课程截止日期
        }elseif ($this->type == 1) {
            $model->deadline_time       = (int)strtotime($this->create_at) + (int)($this->valid_time)*24*60*60;
        }
        $model->overage_section         = $this->courseAmount;                      //剩余节数
        $model->create_at               = (int)strtotime($this->create_at);                                   //买课时间
        ...
        $model->activeTime              = (int)strtotime($this->create_at);                                   //激活时间
    ...
    public function saveConsumptionHistory()
    {
        ...
        $history->type                  = (int)1;
        $history->consumption_date      = (int)strtotime($this->create_at);
        $history->consumption_time      = (int)strtotime($this->create_at);
    ...
    public function saveChargeClassService()
    {
        ...
        if(!empty($server)){
            foreach ($server as $key => $value) {
                ...
                $giftServer->create_time     = (int)strtotime($this->create_at);
    ...
    public function saveOrder()
    {
        ...
        $order->order_time = (int)strtotime($this->create_at);
        $order->pay_money_time = (int)strtotime($this->create_at);
    ...
    public function saveGiftRecord()
    {
        ...
        if (isset($limit) && !empty($limit)) {
            foreach ($limit as $k=>$v) {
                ...
                $gift->create_at      = (int)strtotime($this->create_at);
                ...
                if($this->giftType == 1){
                    ...
                    $gift->get_day = (int)strtotime($this->create_at);
                }
                if(!$gift->save()){
                    return $gift->errors;
                }
    ...
    public function saveChargeClassNumber()
    {
        ...
        if ($number->surplus - 1 == 0) {
            if ($number->surplus_sale_num == 1) {
                $number->surplus_sale_num = 0;
            }else{
