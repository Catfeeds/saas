...
class EntryRecord extends \common\models\base\EntryRecord
{
    ...
    public function getEntryArr($id, $venueId)
    {
        $query = EntryRecord::find()
            ...
            ->joinWith(['memberCard m'])
            ->where(['member_card_id' => $id])
            ->andWhere(['er.venue_id' => $venueId])
    ...
    public function getEntryArrSum($id, $venueId)
    {
        ...
        $data = EntryRecord::find()
            ...
            ->select('er.*,on.name')
            ->joinWith(['organization on'])
            ->where(['er.member_card_id' => $id, 'er.venue_id' => $venueId])
            ->andWhere(['between', 'er.entry_time', $start, $end])
            ->count();
     ...
     * @return bool|string
     */
    public function EntryRecordData($params)
    {
        ...
        if (!empty($params['entryTime'])) {
            ...
            $model = EntryRecord::find()
                ...
                ->select(["er.id", "er.member_card_id", "er.venue_id", "organization.name", "memberCard.card_name", "IFNULL(er.leaving_time,'lt') as leaving_time", "IFNULL(er.entry_time,'et') as entry_time", "round((er.leaving_time-er.entry_time)/3600,1) as abc"])
                ->asArray();
    ...
    {
        $data = EntryRecord::find()->where(['member_id' => $memberId])->groupBy('id')->orderBy(['entry_time' => SORT_DESC])->asArray();
