...
class GroupClass extends \common\models\GroupClass
{
     ...
     */
    public function getGroupClassData($id, $type, $venueId)
    {
        $this->venue = $venueId;
        $memberCard = MemberCard::find()->where(['member_id' => $id])->asArray()->all();
        $memberCardIdArr = array_column($memberCard, 'id');
        // 修改会员约客状态
        $this->updateMemberAboutStatus($id);
        $query = \backend\models\AboutClass::find()
            ...
            ->joinWith(["seat"])
            ->joinWith(["employee"])
            ->where(["cloud_about_class.member_card_id" => $memberCardIdArr])
            ->andWhere(['cloud_about_class.type' => 2])
            ->groupBy(['cloud_about_class.id'])
        ...
        if ($type == 'pot') {
            $query = \backend\models\AboutClass::find()
                ...
                ->joinWith(["groupClass gc" => function ($query) {
                    $query->joinWith(["course course", "classroom cr", "organization or"], false);
                }], false)
                ->joinWith(["seat seat"], false)
                ->joinWith(["employee ee"], false)
                ->where(["about.member_id" => $id])
                ->andWhere(['about.type' => 2])
                ->select('about.id,about.class_id,about.start,about.end,about.status,or.name as oName,course.name as courseName,cr.name as className,seat.seat_number')
                ->groupBy(['about.id'])
                ->orderBy(['about.id' => SORT_DESC])
                ->asArray();
...
//        AboutClass::updateAll(["status"=>4],["and",["<","end",time()],["is_print_receipt"=>1],["member_id"=>$id],["type"=>2],["!=","status",2]]);                                // 下课
        //搜索所有状态为1 3 的课程
        $data = \common\models\AboutClass::find()->alias('about')
            ->select('about.id,about.class_id,class.venue_id,class.company_id,about.start,about.end,about.is_print_receipt,about.in_time,about.member_card_id')
            ->joinWith(['groupClass class'], false)
            ->where(['and',
                ['in', 'about.status', [1, 3]],
                ['about.member_id' => $id],
                ['about.type' => 2]
     ...
     * @return boolean
     */
    public function getClassData($param, $venueIds)
    {
        $this->autoLoad($param, $venueIds);
    ...
    public function getClassDatalist($param, $venueId, $start)
    {
        $this->autoLoad($param, $venueId);
        $this->memberId = $param["memberId"];
        $this->getTimeWeek($param);
        $query = GroupClass::find()
            ->alias("groupClass")
            ...
            ->asArray();
        if (!empty($start)) {
            $query->where(['groupClass.class_date' => $start]);
        }
        if (!empty($this->venueId)) {
            $query = $query->andFilterWhere(["groupClass.venue_id" => $this->venueId]);
        }
    ...
    public function OperationSituation($data, $venueId)
    {
        switch ($data["status"]) {
            ...
            case 3:
                $result = "已下课";
                //对会员的课程状态修改,教练上下课不影响会员
                /*$model = new \backend\models\AboutClass();
                $model->memberClassStatus('','class',$data['id']);*/
    ...
    public function censusClassSituation($id, $venueId)
    {
        ...
        $this->updateMemberRecord($memberCard, $venueId, $id);
    }
    ...
    {
        //爽约
        $leaveIds = AboutClass::find()->where(['and', ['type' => 2], ['class_id' => $id], ['status' => [1, 3]], ['and', ['is_print_receipt' => 2], ['in_time' => 0]]])->asArray()->column();
        //正常下课
        $commonIds = AboutClass::find()->where(['and', ['type' => 2], ['class_id' => $id], ['status' => [1, 3]], ['or', ['is_print_receipt' => 1], ['<>', 'in_time', 0]]])->asArray()->column();
        // 批量修改 会员上课状态  卡没冻结旷课
        if (!empty($leaveIds)) {
            $absent1 = AboutClass::updateAll(["status" => 6], ["and", ["id" => $leaveIds], ["<>", "status", 2]]);           // 修正为旷课状态   旷课
        }
        // 批量修改为 已下课
        if (!empty($commonIds)) {
            $hadClass = AboutClass::updateAll(["status" => 4], ["and", ["id" => $commonIds], ["<>", "status", 2]]);          // 批量修改会员下课状态
        }
    }
     ...
     */
    public function updateMemberRecord($memberCardId, $venueId, $id)
    {
        ...
        //获取当月+1和第一的会员id
        $arr = $this->judgeMemberMissClass($memberCardId, $id);
        if (!empty($arr)) {
            $ids1 = $arr[0];
            $ids2 = $arr[1];
        } else {
        ...
        // 将所有爽约会员的爽约次数+1
        MemberCard::updateAllCounters(["absentTimes" => 1], ["id" => $ids1]);
        //所有爽约会员次数为1
        MemberCard::updateAll(["absentTimes" => 1], ["id" => $ids2]);
    ...
    public function seatType($seatId)
    {
        ...
        if (!empty($seat)) {
            foreach ($seat as $key => $value) {
                $about = AboutClass::find()->where(['and', ['class_id' => $value['id']], ['!=', 'status', 2], ['type' => 2], ['>', 'end', time()]])->one();
                if (isset($about)) {
                    return true;
                }
            }
     ...
     * @return array
     */
    public function judgeMemberMissClass($arr, $id)
    {
        if (isset($arr) && !empty($arr)) {
            $ids1 = [];             //爽约次数+1的会员
            $ids2 = [];             //爽约次数改为1的会员
            foreach ($arr as $k => $v) {
                $result = AboutClass::find()->where(['and', ['status' => 6], ['type' => 2], ['member_card_id' => $v], ['<>', 'class_id', $id]])->orderBy('start desc')->asArray()->one();
                if (!empty($result)) {
                    //不为空判断
                    if (date('m', $result['start']) == date('m', time())) {
                        //当前月+1
                        $ids1[] = $v;
                    } else {
                        //不是当前月爽约次数改为1
     ...
     * @return array
     */
    public function groupClassInfo($venueId)
    {
        $data = GroupClass::find()
            ->alias('gc')
            ->where(['and',
            ...
            ->joinWith(['employee ee'], false)
            ->select('gc.start,gc.end,gc.class_date,course.name as courseName,cr.name as classRoom,ee.name as coachName')
            ->orderBy(['gc.start' => SORT_ASC])
            ->asArray()
            ->all();
        return $data;
    }
