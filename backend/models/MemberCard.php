...
class MemberCard extends \common\models\MemberCard
{
    ...
    {
        self::memberGroupClass($MemberId);     //对当月团课进行处理
        if ($type == 1) {
...
//                ->joinWith(['venueLimitTimesArr venueLimitTimesArr'])
                ...
                ->joinWith(['leaveRecordOneMemberCard le'])
                ->where(['mc.member_id' => $MemberId])
                ->orderBy([new \yii\db\Expression('FIELD (mc.status, 1,4,3,2,6,5)')])
//                ->addOrderBy("mc.invalid_time DESC")
                ->asArray()
                ->all();
        } else {
            $data = time();
...
//        $dataProvider              =  Func::getDataProvider($model,12);
//        $dataProvider->models     =  $this->getEmployeeData($dataProvider->models);
//        $data = $dataProvider->models;
        foreach ($model as $k => $v) {
            $model[$k]['goVenue'] = $this->getGoVenue($v['id'], $venueId);
        }
//        $dataProvider->models = $data;
        //var_dump($data);die;
        //$dataProvider->models     =  $this->calculateInv/alidTime($dataProvider->models);
        return $model;
    }

    /**
     * 后台会员卡管理 根据卡号id获取其直通场馆列表
     * @author Huang hua <huanghua@itsports.club>
     * @create 2017/4/18
     * @return object
     */
    public function getGoVenue($memberCardId, $venueId)
    {
        $data = [];
        if (!$memberCardId) return $data = [];
        $venArr = [];
        $data = \common\models\VenueLimitTimes::find()
            ->alias('vlt')
            ->where(['member_card_id' => $memberCardId])
            ->joinWith(["organization org"], false)
            ->select("vlt.venue_id,vlt.venue_ids,org.name,org.identity")
            ->asArray()->all();

        if ($data) {
            foreach ($data as $v) {
                $venArr = $v['venue_ids'] ? json_decode($v['venue_ids']) : [];
                array_push($venArr, $v['venue_id']);
            }
        }


        return array_intersect($venueId, $venArr);
    ...
    public function getCheckCard($id, $venueId = '')
    {
        ...
        $query = \backend\models\Member::find()
            ...
            ->select([
                ...
                'md.note',
                'md.family_address',
                'mc.id as memberCard_id',
                'ic.custom_ic_number',
                'mc.pid as memberCard_pid',
                'mc.card_category_id',
                'mc.total_times',
                'mc.consumption_times',
                'mc.create_at',
                'mc.active_limit_time',
                'mc.card_number',
                'mc.status as mc_status',
                'mc.recent_freeze_reason',
                'mc.invalid_time',
                'mc.active_time',
                ...
                'md.pic',
                'mc.absentTimes as absent_times',
                'mc.leave_type',
                'org.name as venueName'
            ])
            ->joinWith(['memberDetails md'], false)
            ->joinWith(['icBindingRecordS ic'], false)
            ...
            ->joinWith(['organization org'], false)
            ->asArray()->one();
        $query['invalid_time'] = Func::getTheEndTimeStamp($query['invalid_time']);
        if ($query['mc_status'] == 2) {
            //会员卡升级异常
            $con = \common\models\base\ConsumptionHistory::find()
                ->where(['and',
                    ['or', ['like', 'describe', '由' . $query['cardName'] . '升级为'], ['like', 'consume_describe', '由' . $query['card_number'] . '升级为']],
                    ['consumption_type' => 'card'],
                    ['category' => '升级'],
                    ['member_id' => $query['id']]
                ])->asArray()->one();
            $conSpan = \common\models\base\ConsumptionHistory::find()
                ->where(['and',
                    ['or', ['like', 'describe', '由' . $query['cardName'] . '升级为'], ['like', 'consume_describe', '由' . $query['card_number'] . '升级为']],
                    ['consumption_type' => 'card'],
                    ['category' => '跨店升级'],
            ...
            if (isset($con)) {
                ...
                $query['s_type'] = 1;//普通升级
            } elseif (isset($conSpan)) {
                //$consumDescribe = json_decode($con['consume_describe']);
                $query['newCardNum'] = mb_substr(strrchr($conSpan['consume_describe'], "为"), 1, -1);
                $query['s_type'] = 2;//跨店升级
            } else {
                $query['newCardNum'] = null;
            }
            //会员卡退费异常
            $quit = \common\models\base\ConsumptionHistory::find()
                ->where(['and',
                    ['member_id' => $query['id']],
                    ['consumption_type' => 'card'],
                    ['consumption_type_id' => $id],
                    ['category' => '会员卡退费'], ['like', 'describe', '会员卡退费']
                ])->asArray()->one();
            if (isset($quit) && !empty($quit)) {
                //退费状态 1:退费
                $query['quit_status'] = 1;
            } else {
                $query['quit_status'] = null;
            }
        }
        $this->autoLoad('d');
...
//        $return['list'] = array_slice($data, ($pages - 1) * 5, 5);
    ...
    public function checkMemberCardNumber($num, $venueId)
    {
        ...
        if ($check && !empty($check)) {
            if (isset($check->member_id)) {
                $this->memberGroupClass($check->member_id);
            }
...
//                     }
    ...
    {
        self::memberGroupClass($memberId);     //对团课进行处理
...
//         }
     ...
     * @param  $number //会员卡号
     * @param   $name //会员卡名
     * @return integer   //会员卡信息
     */
    public function setMemberIdentify($number, $name)
    {
        $vipArr = ArrayConfig::setVipCardNameArr();
        if (preg_match('/^80[0-9]*$/', $number) ||
            preg_match('/^80[0-9]*[-][A-Z][0-9]*$/', $number) ||
            preg_match('/^2[0-9]*$/', $number) ||
            preg_match('/^100[0-9]*$/', $number) ||
            preg_match('/^01600[0-9]*$/', $number)) {                                        //正则匹配存在的会员卡号（1000xxxx 尊黑）都可以预约
    ...
    public function getMemberCard($id, $venueId)
    {
        $data = MemberCard::find()->select("id,card_name,status,invalid_time,card_category_id,venue_id,company_id,card_number")->where(['and', ["member_id" => $id], ["venue_id" => $venueId]])->andWhere(['IS', 'pid', null])->asArray();
        $data = $data->all();
...
//        }
     ...
     * @return array
     */
    public function cardCountDetails($params)
    {
        $this->autoLoad($params);
        $entry = \backend\models\EntryRecord::find()
            ->alias('er')
            ->joinWith(['memberCard mc' => function ($entry) {
                $entry->joinWith(['cardCategory cc' => function ($entry) {
                    $entry->joinWith(['cardCategoryType cct'], false);
                }], false);
            }], false)
            ->joinWith(['member mm' => function ($entry) {
                $entry->joinWith(['memberDetails md'], false);
            }], false)
            ->select('er.id,er.member_id,er.entry_time,cct.id as category_type,cct.type_name,mc.card_name,mc.card_number,mc.type,mm.mobile,md.name,md.sex')
            ->asArray();
        $entry = $this->getSearchWhere($entry);
        $data = Func::getDataProvider($entry, 8);
        return $data;
    }
    ...
    public function autoLoad($param)
    {
        ...
        $this->categoryType = (isset($param[self::CATEGORY_TYPE]) && !empty($param[self::CATEGORY_TYPE])) ? $param[self::CATEGORY_TYPE] : null;
        $this->cardType = (isset($param[self::CARD_TYPE]) && !empty($param[self::CARD_TYPE])) ? $param[self::CARD_TYPE] : null;
        $this->cardNumber = (isset($param[self::CARD_NUMBER]) && !empty($param[self::CARD_NUMBER])) ? $param[self::CARD_NUMBER] : null;
        $this->keywords = (isset($param[self::KEYWORDS]) && !empty($param[self::KEYWORDS])) ? $param[self::KEYWORDS] : null;
    }

    /**
     * @营运统计 - 会员卡种统计 - 查看详情 - 执行相应搜索
     * @author zhumengke <zhumengke@itsports.club>
     * @create 2018/05/07
     * @return array
     */
    ...
    {
        $query->andFilterWhere(["and", [">=", "er.entry_time", $this->theDateStart], ["<=", "er.entry_time", $this->theDateEnd]]);
        if (empty($this->anotherVenue)) {
        ...
        $query->andFilterWhere(["cct.id" => $this->categoryType]);
        $query->andFilterWhere(["mc.type" => $this->cardType]);
        $query->andFilterWhere(["mc.card_number" => $this->cardNumber]);
        $query->andFilterWhere(["or",
            ["mc.card_name" => $this->keywords],
...
//        return $data;
    ...
    public function thawMemberCard($memberCardId, $venueId, $price)
    {
        ...
        $model->last_freeze_time = time();  // 最后一次冻结时间(团课冻结,解冻时由冻结时间改为解冻时间)
        if (!$model->save()) {
            return $model->errors;
        }
        return true;
    ...
    public function memberCardDel($memberCardId)
    {
        ...
        //查询副卡
        $sonCaed = MemberCard::find()->where(['pid' => $memberCardId])->asArray()->count();
        if ($sonCaed) {
            return '请先删除副卡!';
        }
        ...
        //判断是否满足删除条件
        //1.约过团课
        $league = \backend\models\AboutClass::find()->where(['member_card_id' => $memberCardId, 'type' => 2])->asArray()->count();
        if ($league > 0) {
            return '此会员卡预约过团课,不能删除!';
        }
        //2.购买私课
        $class = MemberCourseOrder::find()->where(['member_card_id' => $memberCardId])->asArray()->count();
        if ($class > 0) {
            return '此会员卡购买或赠送过私课,不能删除!';
        }
        //3.进馆
        $entryRecord = \backend\models\EntryRecord::find()->where(['member_card_id' => $memberCardId])->asArray()->count();
        if ($entryRecord > 0) {
            return '此会员卡存在进馆记录,不能删除!';
        }
        //4.预约场地
        $yard = AboutYard::find()->where(['member_card_id' => $memberCardId])->asArray()->count();
        if ($yard > 0) {
            return '此会员卡预约过场地,不能删除!';
        }
        //5.请假
        $leave = \backend\models\LeaveRecord::find()->where(['member_card_id' => $memberCardId])->asArray()->count();
        if ($leave > 0) {
            return '此会员卡存在请假记录,不能删除!';
        }
        //6.柜子,租柜历史消费记录
        $cabinet = MemberCabinetRentHistory::find()->where(['member_card_id' => $memberCardId])->asArray()->count();
        if ($cabinet > 0) {
            return '此会员卡存在租柜消费,不能删除!';
        }
        ...
        $memberCardAll = MemberCard::find()->where(['member_id' => $memberCard['member_id']])->asArray()->all();
        ...
        try {
            ...
            //2.一张卡的会员修改为过期会员
            if (count($memberCardAll) == 1) {
                $member->member_type = 3;
                $resultMem = $member->save();
                if (!$resultMem) {
                    return "修改会员信息失败!";
                }
            }
            ...
            //3.删除历史消费记录表信息
            $history = ConsumptionHistory::find()->where(["consumption_type_id" => $memberCardId, "consumption_type" => 'card'])->asArray()->count();
            if ($history) {
                $resultHistory = ConsumptionHistory::deleteAll(["consumption_type_id" => $memberCardId, "consumption_type" => 'card']);
                if (!$resultHistory) {
                    return "删除历史消费记录信息失败!";
                }
            }
            ...
            //4.删除订单
            $order = Order::find()->where(["consumption_type_id" => $memberCardId, "consumption_type" => 'card'])->asArray()->count();
            if ($order) {
                $resultOrder = Order::deleteAll(["consumption_type_id" => $memberCardId, "consumption_type" => 'card']);
                if (!$resultOrder) {
                    return '删除会员卡订单失败!';
                }
            }
            ...
            //续费删除历史消费记录
            $cardRenewHistory = ConsumptionHistory::find()->where(["consumption_type_id" => $memberCardId, "consumption_type" => 'cardRenew'])->asArray()->count();
            if ($cardRenewHistory) {
                $resultHistory = ConsumptionHistory::deleteAll(["consumption_type_id" => $memberCardId, "consumption_type" => 'cardRenew']);
                if (!$resultHistory) {
                    return "删除历史消费记录信息失败!";
                }
            }
            ...
            //续费删除订单
            $cardRenewOrder = Order::find()->where(["consumption_type_id" => $memberCardId, "consumption_type" => 'cardRenew'])->asArray()->count();
            if ($cardRenewOrder) {
                $resultOrder = Order::deleteAll(["consumption_type_id" => $memberCardId, "consumption_type" => 'cardRenew']);
                if (!$resultOrder) {
                    return '删除会员卡订单失败!';
                }
            }
            ...
            //5.删除会员卡时间表
            $cardTimeCount = MemberCardTime::find()->where(['member_card_id' => $memberCardId])->asArray()->count();
            if ($cardTimeCount) {
                $cardTime = MemberCardTime::deleteAll(['member_card_id' => $memberCardId]);
                if (!$cardTime) {
                    return "操作失败!";
                }
            }
            ...
            //6.删除会员卡绑定套餐记录
            $bindCount = BindMemberCard::find()->where(['member_card_id' => $memberCardId])->asArray()->count();
            if ($bindCount) {
                $bind = BindMemberCard::deleteAll(['member_card_id' => $memberCardId]);
                if (!$bind) {
                    return "操作失败!";
                }
            }
            ...
            //7.删除进馆场次核算表
            $limitCount = VenueLimitTimes::find()->where(['member_card_id' => $memberCardId])->asArray()->count();
            if ($limitCount) {
                $limitTimes = VenueLimitTimes::deleteAll(['member_card_id' => $memberCardId]);
                if (!$limitTimes) {
                    return "操作失败!";
                }
            }
    ...
    public function checkMemberInfo($cardNumber, $venueId)
    {
        $data = \common\models\MemberCard::find()
            ->alias('mc')
            ->select('mc.id,mc.member_id,mds.name,mds.sex,mds.id_card,mds.pic,mds.birth_date,mm.mobile,mc.invalid_time,mc.status,mc.create_at')
            ->where(['mc.card_number' => $cardNumber])
     ...
     * @return:
     */
    public static function getMemberCardStatus($cardId)
    {
        $card = MemberCard::findOne(['id' => $cardId]);
        if (empty($card)) {
            return '会员卡不存在';
        }
        $leave = LeaveRecord::find()->where(['member_card_id' => $cardId, 'type' => 2, 'status' => 1])->asArray()->one();
        if (isset($leave)) {
            return '此卡正在请假中';
        }
        switch ($card['status']) {
            case 1:
                if ($card['invalid_time'] >= time()) {
                    return true;
                } else {
                    return '会员卡已过期';
                }
...
//        if($card['status'] == 1 && $card['invalid_time'] >= time()){
//            return true;
//        }elseif($card['status'] == 1 && $card['invalid_time'] < time()){
//            return '会员卡已过期';
...
//        }
    ...
    /**
     * 后台会员管理 - 会员卡详情 - 点击出发自动修改会员卡到期状态
     * @author huanghua <huanghua@itsports.club>
     * @create 2018/5/8
     * @param $memberId
     * @return bool
     */
    public function memberCardUpdate($memberId)
    {
        $memberCardAll = MemberCard::find()->where(['and', ['<', 'invalid_time', time()], ['member_id' => $memberId]])->andWhere(['status' => 1])->asArray()->all();
        $memberCardAllTwo = MemberCard::find()->where(['and', ['>', 'invalid_time', time()], ['member_id' => $memberId], ['status' => 6]])->asArray()->all();
        if (!empty($memberCardAll) && !empty($memberCardAllTwo)) {
            foreach ($memberCardAll as $k => $v) {
                $memberCardOne = MemberCard::findOne(['id' => $v['id']]);
                $memberCardOne->status = 6;
                if ($memberCardOne->save() != true) {
                    return $memberCardOne->errors;
                }
            }
            foreach ($memberCardAllTwo as $k => $v) {
                $memberCardTwo = MemberCard::findOne(['id' => $v['id']]);
                $memberCardTwo->status = 1;
                if ($memberCardTwo->save() != true) {
                    return $memberCardTwo->errors;
                }
            }
            return true;
        } else if (!empty($memberCardAll) && empty($memberCardAllTwo)) {
            foreach ($memberCardAll as $k => $v) {
                $memberCardOne = MemberCard::findOne(['id' => $v['id']]);
                $memberCardOne->status = 6;
                if ($memberCardOne->save() != true) {
    ...
    /*
     * @desc: 业务后台 - 筛选会员,会员卡,卡种场馆不符的数据
     * @author: 付钟超 <fuzhongchao@itsports.club>
     * @create: 2018/03/15
     * @return string
     */
    public function selectNotSameVenue($venueId)
    {
        $sql = '(`card`.`venue_id` <> `category`.`venue_id`)';
        $data = MemberCard::find()
            ->alias('card')
            ->joinWith(['member member' => function ($query) {
                $query->joinWith(['memberDetails memberDetail'], false);
            }], false)
            ->joinWith(['cardCategory category'], false)
            ->where($sql)
            ->select('card.id,member.venue_id as member_venue_id,card.venue_id as card_venue_id,category.venue_id as category_venue_id,member.id as member_id,card.id as member_card_id,category.id as card_category_id,memberDetail.name,card.card_number,category.card_name')
            ->andWhere(['and',
    ...
    public function memberGroupClass($memberId = '')
    {
        //获取当月所有团课信息处理
        $beginThisMonth = mktime(0, 0, 0, date('m'), 1, date('Y'));
        $data = \common\models\AboutClass::find()->where(['and', ['>=', 'start', $beginThisMonth], ['type' => 2], ['status' => [1, 3]], ['member_id' => $memberId]])->asArray()->all();
        if (isset($data) && !empty($data)) {
            //对获取的课程进行判断课程状态
            $model = new \backend\models\AboutClass();
            $model->memberClassStatus($data, 'member', '');
        }
    }
     ...
     * @return bool|string
     */
    public function updateAllCardStatus()
    {
        MemberCard::updateAll(['status' => 1], ['and', ['status' => 6], ['>', 'invalid_time', time()]]);
        return true;
    }
}