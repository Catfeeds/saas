...
class GroupCourse extends Model
{
    ...
    public function getSearchWhere($query, $status)
    {
        ...
        if(!empty($status)){
            $query->andFilterWhere(['course.company_id' => \backend\rbac\Config::accessCompany()]);
        }else{
            $query->andFilterWhere(['course.venue_id' => \backend\rbac\Config::accessVenues()]);
        }
    ...
    /**
     * @describe 后台 - 新团课 - 点击修改的时候获取课种初始化数据
     ...
     * @return array
     */
    public function classType($id, $companyIds)
    {
        $model = new  \backend\models\Course();
        $courseType = $model->getClassDataS(2, $companyIds);
