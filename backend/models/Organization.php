...
class Organization extends \common\models\base\Organization
{
    ...
    public static function getOrganizationVenue($id)
    {
     ...
     * @update 2017-4-21
     */
    public function getOrganizationOption($id, $status = null, $companyId = null, $identity = null)
    {
        ...
        if (!empty($status)) {
            ...
            $venue = $this->filter($venue, $id);                    // 针对卡种以外的场馆筛选
        }
    ...
    public function filterMemberCard($venue, $companyId)
    {
        ...
        // 获取公司联盟下的所有公司
        $allCompanyData = ApplyRecord::find()->where([
            "and",
            ["apply_id" => $companyId],
            [">", "end_apply", time()],
            ["status" => 1]
     ...
     * @return mixed
     */
    public function filter($venue, $id)
    {
        // 如果场馆信息不为空 增加帅选
     ...
     * @update 2017-5-9
     */
    public function getOrganizationDepartment($id)
    {
     ...
     * @update 2017-5-9
     */
    public function getOrganizationDepartmentDate($id, $companyId)
    {
        $department = Organization::find()
            ->alias('or')
            ->joinWith(['organization org'])
    ...
    /**
     * @describe 后台 - 组织架构管理 - 处理前台发送过来的搜索参数数据时
     * @author <yanghuilei@itsport.club>
     * @createAt 2018-07-31
     * @param $data
     * @return bool
    ...
    {
        $this->venueId = (isset($data[self::VEN]) && !empty($data[self::VEN])) ? $data[self::VEN] : \backend\rbac\Config::accessVenues();
    ...
    public function getMyData($params)
    {
        ...
        $query = Organization::find()
            ...
            ->andWhere(['cloud_organization.id' => \backend\rbac\Config::accessCompany()])
            ->asArray();
     ...
     */
    public function getAllIdentifyData($companyIds)
    {
        $company = \common\models\base\Organization::find()->where(["style" => 1]);
        $company = $company->andWhere(["id" => $companyIds])->asArray()->all();
     ...
     * @return object
     */
    public function roomInfo($venueId)
    {
        $data = Organization::find()
            ->where(['style' => 4, 'pid' => $venueId])
    ...
    public function roomlist($venueId)
    {
        $data = self::room($venueId);
        return $data;
    }

    private function room($venueId)
    {
        $query = Organization::find()
            ->where(['style' => 4, 'pid' => $venueId])
            ->select('id,name,pid,code')
    ...
    public function venueName($venueId)
    {
        $list = Organization::find()
            ->where(['id' => $venueId])
            ->select('name as venuename')
            ->asArray()
            ->one();
        return $list;
    }
