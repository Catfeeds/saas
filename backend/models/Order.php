...
class Order extends \common\models\Order
{
    ...
    const APP_BUY = 'appBuy';

    /**
     * @describe @云运动 - 后台 - 查询所有订单
    ...
    public function getOrderInfo($params)
    {
        ...
        $query = Order::find()
            ...
            ee.name as createName,
            eeS.name as sellName,
            cc.single,md.pic
             ")
            ->orderBy($this->sorts)
            ->groupBy('or.id')
    ...
    public function getCardOrderInfo($params, $isNotDisplay = "")
    {
        ...
        $query = Order::find()
            ...
            ->joinWith(["organization org"], false)
            ->joinWith(["employee ee"], false)
            ->where(['in', 'or.note', ['售卡', '续费', '升级', '办卡', '迈步一体机售卡', '回款']]);
        ...
        $query = \backend\models\MemberCourseOrder::find()->alias("mco")
            ...
            ->select(
                "mco.*,
                 ...
                 order.order_time,
                 order.total_price as orderMoney,
                ")
            ->orderBy($this->sorts)
            ->where([
                'or',
    ...
    public function setWhereSearch($query)
    {
        ...
        if ($this->appBuy == 1) {
            $query->andFilterWhere(['or.sell_people_id' => 0]);
        }
            ...
            switch ($this->businessBehavior) {
                case 1 :
                    $query->andFilterWhere([
                        ...
                        ['or', ['or.note' => '私教产品'], ['or.note' => 'app私课购买'], ['or.note' => '购买私课']]
                    ]);
                    break;
                case 2 :
                    $query->andFilterWhere([
                ...
                case 10 :
                    $query->andFilterWhere([
                        'not in', 'or.note', ['私教产品', '私教小团体', '办卡', '续费', '升级', '租柜', '续柜', 'app私课购买', '购买私课', '跨店升级', '私课转课']
                    ]);
                    break;
            }
        }
    ...
    public function customLoad($data)
    {
        ...
        $this->appBuy = (isset($data[self::APP_BUY]) && !empty($data[self::APP_BUY])) ? (int)$data[self::APP_BUY] : null;
        return true;
    }
    ...
    public function getOrderOneData($id)
    {
        $data = Order::find()
            ...
            ->joinWith(["employee ee"], false)
            ->joinWith(["employeeS ees"], false)
            ->joinWith(["employeeA eea"], false)
            ->select("or.*,org.name as venue_name,ee.name as createName,ees.name as sellName,eea.name as approvalName")
            ->where(["or.id" => $id])
            ->asArray()
            ->one();
        return $data;
    ...
    public function getOrderData($id)
    {
        $data = Order::find()
            ->alias("or")
            ->joinWith(["memberCard mc" => function ($query) {
                $query->joinWith(['cardCategory cc' => function ($query) {
                    $query->joinWith(['organization ccVenue'], false);
                }], false);
                $query->joinWith(['organization venue'], false);
            }], false)
            ->joinWith(["member mm"], false)
            ->joinWith(["memberDetails md"], false)
            ->select("or.*,md.pic,md.sex,mm.username,mm.id as mem_id,mm.mobile,cc.duration,md.birth_date,md.note as mdNote,mc.card_number,mc.active_time,mc.invalid_time,venue.name as vName,ccVenue.name as ccName,cc.single,mc.note as mcNote")
            ->where(["or.id" => $id])
            ->asArray()
            ->one();
        return $data;
...
//            $order->payee_id        =  \Yii::$app->user->identity->id;          // 收款人id
    ...
    public function setWhere($query)
    {
        ...
        if ($this->searchGood == 'card') {
            $query->andFilterWhere(['in', 'cloud_order.note', ['购卡', '售卡', '迈步一体机售卡', '办卡', '续费', '升级', '迈步一体机办卡']]);
        } elseif ($this->searchGood == 'charge') {
            $query->andFilterWhere(['in', 'cloud_order.note', ['app私课购买', '私教产品', '私教小团体', '私课续费']]);
        } elseif ($this->searchGood == 'cabinet') {
            $query->andFilterWhere(['in', 'cloud_order.note', ['柜子押金', '租柜', '续柜']]);
        } elseif ($this->searchGood == 'other') {
            $query->andFilterWhere(['not in', 'cloud_order.note', ['购卡', '售卡', '迈步一体机售卡', '办卡', '续费', '升级', '迈步一体机办卡', 'app私课购买', '私教产品', '私教小团体', '私课续费', '柜子押金', '租柜', '续柜']]);
        }
        $query->andFilterWhere([
            'and',
    ...
    public function refuseApply($data)
    {
        ...
        if (!empty($order)) {
            $adminModel = Employee::findOne(['admin_user_id' => \Yii::$app->user->identity->id]);
            $order->approval_id = isset($adminModel->id) ? intval($adminModel->id) : 0;         //审批人id
            $order->status = 6;                           //拒绝申请
    ...
    public function getOtherInfo($params)
    {
        ...
        $query = Order::find()
            ...
            ->orderBy($this->sorts)
            ->where(['in', 'or.note', ['租柜', '会员卡转卡', '订金', '手环工本费', '续柜', '柜子押金', '定金', '课升级定金', '卡升级定金', '购卡定金', '购课定金', '续费定金', '私课转课']])
            ->andWhere(['not in', 'or.status', [1, 5]])
            ->asArray();
    ...
    public function setOtherWhereSearch($query)
    {
        ...
        if ($this->note == '定金') {
            $query->andFilterWhere(['in', 'or.card_name', ['定金', '订金']]);
        } else {
            $query->andFilterWhere(['or.note' => $this->note]);
        }
    ...
    public function getOrderDel($id)
    {
        ...
        $history = ConsumptionHistory::find()
            ...
            ->andWhere(['consumption_type' => $order->consumption_type])
            ->andWhere(['consumption_type_id' => $order->consumption_type_id])
            ->andFilterWhere(['venue_id' => $order->venueId])
            ->andFilterWhere(['consumption_amount' => $order->net_price])
            ->one();
        if (isset($history)) {
            $history->delete();
        }
        if ($order->consumption_type == 'deposit') {
            $deposit = MemberDeposit::findOne(['id' => $order->consumption_type_id]);
            $deposit->delete();
        }
    ...
    public function updateOrder($param)
    {
        ...
        if ($order->consumption_type == 'charge' && $order->note != '私课转课') {
     ...
     * @return  mixed
     */
    public function updateOrderSellPeople($param)
    {
        if (empty($param["orderId"])) {
            return false;
        }
        ...
        $order = \common\models\base\Order::findOne($param["orderId"]);
        //var_dump($order->sell_people_name);die;
        // 获取所属公司id
        $company = Organization::findOne($param["venueId"]);
        // 获取所属公司员工姓名
        $employee = Employee::findOne($param["employeeId"]);
