        ...
        $query = MemberCourseOrder::find()
            ...
            ->select(
                "order.id,
                 ...
                 memberCourseOrderDetails.id as orderDetailsId,
                 memberCourseOrderDetails.deal_id,
...
//            ->andWhere(['chargeClass.venue_id' => $venueId])
            ...
            ->select(["mco.*", "TRUNCATE(mco.money_amount / mco.course_amount,2) as one_price"])
            ->asArray()
            ->all();
        return $data;
    }
    ...
    {
        $cOrder = \common\models\base\MemberCourseOrder::findOne(['id' => $cid]);
        if (empty($cOrder['class_number_id'])) {
...
//        }
    ...
    public function customLoads($data)
    {
        ...
        $this->venueId = (isset($data[self::VENUE_ID]) && !empty($data[self::VENUE_ID])) ? $data[self::VENUE_ID] : Config::accessVenues();
...
//        }
    ...
    public function autoLoad($param)
    {
        ...
        $this->venueId = (isset($param[self::VENUE_ID]) && !empty($param[self::VENUE_ID])) ? $param[self::VENUE_ID] : Config::accessVenues();
    }
